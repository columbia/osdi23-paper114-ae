; ModuleID = 'crypto/des/ofb64ede.c'
source_filename = "crypto/des/ofb64ede.c"
...
define void @DES_ede3_ofb64_encrypt(i8* nocapture noundef readonly %in, i8* nocapture noundef writeonly %out, i64 noundef %length, %struct.DES_ks* noundef %k1, %struct.DES_ks* noundef %k2, %struct.DES_ks* noundef %k3, [8 x i8]* nocapture noundef %ivec, i32* nocapture noundef %num) local_unnamed_addr #0 {
entry:
...
while.body:                                       ; preds = %entry, %if.end
  ...
  %v1.0212 = phi i32 [ %v1.1, %if.end ], [ %or24, %entry ]
  %v0.0211 = phi i32 [ %v0.1, %if.end ], [ %or10, %entry ]
  %in.addr.0210 = phi i8* [ %incdec.ptr90, %if.end ], [ %in, %entry ]
  %out.addr.0209 = phi i8* [ %incdec.ptr95, %if.end ], [ %out, %entry ]
  %dec215 = add nsw i64 %dec215.in, -1
  %cmp = icmp eq i32 %n.0213, 0
  br i1 %cmp, label %if.then, label %if.end

if.then:                                          ; preds = %while.body
  call void @DES_encrypt3(i32* noundef nonnull %arrayidx25, %struct.DES_ks* noundef %k1, %struct.DES_ks* noundef %k2, %struct.DES_ks* noundef %k3) #4
  %11 = load i32, i32* %arrayidx25, align 4, !tbaa !4
  %12 = load i32, i32* %arrayidx26, align 4, !tbaa !4
  %conv61 = trunc i32 %11 to i8
  store i8 %conv61, i8* %1, align 1, !tbaa !8
  %shr63 = lshr i32 %11, 8
  %conv65 = trunc i32 %shr63 to i8
  store i8 %conv65, i8* %incdec.ptr28, align 1, !tbaa !8
  %shr67 = lshr i32 %11, 16
  %conv69 = trunc i32 %shr67 to i8
  store i8 %conv69, i8* %incdec.ptr31, align 1, !tbaa !8
...
if.end:                                           ; preds = %if.then, %while.body
  %v0.1 = phi i32 [ %11, %if.then ], [ %v0.0211, %while.body ]
  %v1.1 = phi i32 [ %12, %if.then ], [ %v1.0212, %while.body ]
  %save.1 = phi i32 [ %inc, %if.then ], [ %save.0214, %while.body ]
  %incdec.ptr90 = getelementptr inbounds i8, i8* %in.addr.0210, i64 1
  %13 = load i8, i8* %in.addr.0210, align 1, !tbaa !8
  %idxprom = sext i32 %n.0213 to i64
  %arrayidx92 = getelementptr inbounds [8 x i8], [8 x i8]* %d, i64 0, i64 %idxprom
  %14 = load i8, i8* %arrayidx92, align 1, !tbaa !8
  %xor206 = xor i8 %14, %13
  %incdec.ptr95 = getelementptr inbounds i8, i8* %out.addr.0209, i64 1
...
while.end:                                        ; preds = %if.end
  ...
  br i1 %tobool97.not, label %if.end130, label %if.then98

if.then98:                                        ; preds = %while.end
  %conv101 = trunc i32 %v0.1 to i8
  store i8 %conv101, i8* %arrayidx, align 1, !tbaa !8
...
if.end130:                                        ; preds = %entry, %if.then98, %while.end
  %n.0.lcssa224 = phi i32 [ %and96, %if.then98 ], [ %and96, %while.end ], [ %0, %entry ]
  store i32 %n.0.lcssa224, i32* %num, align 4, !tbaa !4
