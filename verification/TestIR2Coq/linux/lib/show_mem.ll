...
define dso_local void @show_mem(i32 noundef %filter, %struct.nodemask_t* noundef %nodemask) local_unnamed_addr #0 {
entry:
  %call = call i32 (i8*, ...) @_printk(i8* noundef getelementptr inbounds ([11 x i8], [11 x i8]* @.str, i64 0, i64 0)) #5
  call void @show_free_areas(i32 noundef %filter, %struct.nodemask_t* noundef %nodemask) #6
  %call1 = call %struct.pglist_data* @first_online_pgdat() #6
  %tobool.not53 = icmp eq %struct.pglist_data* %call1, null
  br i1 %tobool.not53, label %do.end19, label %for.cond2.preheader

for.cond2.preheader:                              ; preds = %entry, %cleanup.3
  %pgdat.056 = phi %struct.pglist_data* [ %call15, %cleanup.3 ], [ %call1, %entry ]
  %total.055 = phi i64 [ %total.2.3, %cleanup.3 ], [ 0, %entry ]
  %reserved.054 = phi i64 [ %reserved.2.3, %cleanup.3 ], [ 0, %entry ]
  %arrayidx = getelementptr %struct.pglist_data, %struct.pglist_data* %pgdat.056, i64 0, i32 0, i64 0
  %call4 = call fastcc i1 @populated_zone(%struct.zone* noundef %arrayidx) #7
  br i1 %call4, label %if.end, label %cleanup

if.end:                                           ; preds = %for.cond2.preheader
  %present_pages = getelementptr %struct.pglist_data, %struct.pglist_data* %pgdat.056, i64 0, i32 0, i64 0, i32 12
  %0 = load i64, i64* %present_pages, align 64
  %call6 = call fastcc i64 @zone_managed_pages(%struct.zone* noundef %arrayidx) #7
  %sub = add i64 %0, %reserved.054
  %add7 = sub i64 %sub, %call6
  %add = add i64 %0, %total.055
  br label %cleanup

cleanup:                                          ; preds = %for.cond2.preheader, %if.end
  %reserved.2 = phi i64 [ %add7, %if.end ], [ %reserved.054, %for.cond2.preheader ]
  %total.2 = phi i64 [ %add, %if.end ], [ %total.055, %for.cond2.preheader ]
  %arrayidx.1 = getelementptr %struct.pglist_data, %struct.pglist_data* %pgdat.056, i64 0, i32 0, i64 1
  %call4.1 = call fastcc i1 @populated_zone(%struct.zone* noundef %arrayidx.1) #7
  br i1 %call4.1, label %if.end.1, label %cleanup.1

if.end.1:                                         ; preds = %cleanup
  %present_pages.1 = getelementptr %struct.pglist_data, %struct.pglist_data* %pgdat.056, i64 0, i32 0, i64 1, i32 12
  %1 = load i64, i64* %present_pages.1, align 64
  %call6.1 = call fastcc i64 @zone_managed_pages(%struct.zone* noundef %arrayidx.1) #7
  %sub.1 = add i64 %1, %reserved.2
  %add7.1 = sub i64 %sub.1, %call6.1
  %add.1 = add i64 %1, %total.2
  br label %cleanup.1

cleanup.1:                                        ; preds = %if.end.1, %cleanup
  %reserved.2.1 = phi i64 [ %add7.1, %if.end.1 ], [ %reserved.2, %cleanup ]
  %total.2.1 = phi i64 [ %add.1, %if.end.1 ], [ %total.2, %cleanup ]
  %arrayidx.2 = getelementptr %struct.pglist_data, %struct.pglist_data* %pgdat.056, i64 0, i32 0, i64 2
  %call4.2 = call fastcc i1 @populated_zone(%struct.zone* noundef %arrayidx.2) #7
  br i1 %call4.2, label %if.end.2, label %cleanup.2

if.end.2:                                         ; preds = %cleanup.1
  %present_pages.2 = getelementptr %struct.pglist_data, %struct.pglist_data* %pgdat.056, i64 0, i32 0, i64 2, i32 12
  %2 = load i64, i64* %present_pages.2, align 64
  %call6.2 = call fastcc i64 @zone_managed_pages(%struct.zone* noundef %arrayidx.2) #7
  %sub.2 = add i64 %2, %reserved.2.1
  %add7.2 = sub i64 %sub.2, %call6.2
  %add.2 = add i64 %2, %total.2.1
  br label %cleanup.2

cleanup.2:                                        ; preds = %if.end.2, %cleanup.1
  %reserved.2.2 = phi i64 [ %add7.2, %if.end.2 ], [ %reserved.2.1, %cleanup.1 ]
  %total.2.2 = phi i64 [ %add.2, %if.end.2 ], [ %total.2.1, %cleanup.1 ]
  %arrayidx.3 = getelementptr %struct.pglist_data, %struct.pglist_data* %pgdat.056, i64 0, i32 0, i64 3
  %call4.3 = call fastcc i1 @populated_zone(%struct.zone* noundef %arrayidx.3) #7
  br i1 %call4.3, label %if.end.3, label %cleanup.3

if.end.3:                                         ; preds = %cleanup.2
  %present_pages.3 = getelementptr %struct.pglist_data, %struct.pglist_data* %pgdat.056, i64 0, i32 0, i64 3, i32 12
  %3 = load i64, i64* %present_pages.3, align 64
  %call6.3 = call fastcc i64 @zone_managed_pages(%struct.zone* noundef %arrayidx.3) #7
  %sub.3 = add i64 %3, %reserved.2.2
  %add7.3 = sub i64 %sub.3, %call6.3
  %add.3 = add i64 %3, %total.2.2
  br label %cleanup.3

cleanup.3:                                        ; preds = %if.end.3, %cleanup.2
  %reserved.2.3 = phi i64 [ %add7.3, %if.end.3 ], [ %reserved.2.2, %cleanup.2 ]
  %total.2.3 = phi i64 [ %add.3, %if.end.3 ], [ %total.2.2, %cleanup.2 ]
  %call15 = call %struct.pglist_data* @next_online_pgdat(%struct.pglist_data* noundef nonnull %pgdat.056) #6
  %tobool.not = icmp eq %struct.pglist_data* %call15, null
  br i1 %tobool.not, label %do.end19, label %for.cond2.preheader

do.end19:                                         ; preds = %cleanup.3, %entry
  %reserved.0.lcssa = phi i64 [ 0, %entry ], [ %reserved.2.3, %cleanup.3 ]
  %total.0.lcssa = phi i64 [ 0, %entry ], [ %total.2.3, %cleanup.3 ]
  %call21 = call i32 (i8*, ...) @_printk(i8* noundef getelementptr inbounds ([15 x i8], [15 x i8]* @.str.1, i64 0, i64 0), i64 noundef %total.0.lcssa) #5
  %call26 = call i32 (i8*, ...) @_printk(i8* noundef getelementptr inbounds ([31 x i8], [31 x i8]* @.str.2, i64 0, i64 0), i64 noundef 0) #5
  %call31 = call i32 (i8*, ...) @_printk(i8* noundef getelementptr inbounds ([20 x i8], [20 x i8]* @.str.3, i64 0, i64 0), i64 noundef %reserved.0.lcssa) #5
  ret void
}
...
declare dso_local void @show_free_areas(i32 noundef, %struct.nodemask_t* noundef) local_unnamed_addr #2

; Function Attrs: null_pointer_is_valid
declare dso_local %struct.pglist_data* @first_online_pgdat() local_unnamed_addr #2

; Function Attrs: mustprogress nofree noinline norecurse nosync nounwind null_pointer_is_valid readonly willreturn
define internal fastcc i1 @populated_zone(%struct.zone* nocapture noundef readonly %zone) unnamed_addr #3 {
entry:
...
define internal fastcc i64 @zone_managed_pages(%struct.zone* noundef %zone) unnamed_addr #4 {
entry:
...
; Function Attrs: null_pointer_is_valid
declare dso_local %struct.pglist_data* @next_online_pgdat(%struct.pglist_data* noundef) local_unnamed_addr #2
