...
define dso_local nonnull %struct.pglist_data* @first_online_pgdat() local_unnamed_addr #0 {
entry:
  ret %struct.pglist_data* @contig_page_data
}

; Function Attrs: mustprogress nofree noinline norecurse nosync nounwind null_pointer_is_valid readnone willreturn
define dso_local noalias %struct.pglist_data* @next_online_pgdat(%struct.pglist_data* nocapture readnone %pgdat) local_unnamed_addr #0 {
entry:
  ret %struct.pglist_data* null
}

; Function Attrs: mustprogress nofree noinline norecurse nosync nounwind null_pointer_is_valid readonly willreturn
define dso_local %struct.zone* @next_zone(%struct.zone* noundef readonly %zone) local_unnamed_addr #1 {
entry:
  ...
  %add.ptr1 = getelementptr %struct.pglist_data, %struct.pglist_data* %0, i64 0, i32 0, i64 3
  %cmp = icmp ugt %struct.zone* %add.ptr1, %zone
  %incdec.ptr = getelementptr %struct.zone, %struct.zone* %zone, i64 1
  %spec.select = select i1 %cmp, %struct.zone* %incdec.ptr, %struct.zone* null
  ret %struct.zone* %spec.select
}

; Function Attrs: nofree noinline norecurse nosync nounwind null_pointer_is_valid readonly
define dso_local %struct.zoneref* @__next_zones_zonelist(%struct.zoneref* noundef readonly %z, i32 noundef %highest_zoneidx, %struct.nodemask_t* noundef readnone %nodes) local_unnamed_addr #2 {
entry:
  %cmp = icmp eq %struct.nodemask_t* %nodes, null
  br i1 %cmp, label %while.cond, label %while.cond4, !prof !7

while.cond:                                       ; preds = %entry, %while.cond
  %z.addr.0 = phi %struct.zoneref* [ %incdec.ptr, %while.cond ], [ %z, %entry ]
  %call = call fastcc i32 @zonelist_zone_idx(%struct.zoneref* noundef %z.addr.0) #6
  %cmp2 = icmp ugt i32 %call, %highest_zoneidx
  %incdec.ptr = getelementptr %struct.zoneref, %struct.zoneref* %z.addr.0, i64 1
  br i1 %cmp2, label %while.cond, label %if.end

while.cond4:                                      ; preds = %entry, %while.cond4
  %z.addr.1 = phi %struct.zoneref* [ %incdec.ptr14, %while.cond4 ], [ %z, %entry ]
  %call5 = call fastcc i32 @zonelist_zone_idx(%struct.zoneref* noundef %z.addr.1) #6
  %cmp6 = icmp ugt i32 %call5, %highest_zoneidx
  %incdec.ptr14 = getelementptr %struct.zoneref, %struct.zoneref* %z.addr.1, i64 1
  br i1 %cmp6, label %while.cond4, label %if.end

if.end:                                           ; preds = %while.cond4, %while.cond
  %z.addr.2 = phi %struct.zoneref* [ %z.addr.0, %while.cond ], [ %z.addr.1, %while.cond4 ]
  ret %struct.zoneref* %z.addr.2
}
...
define dso_local void @lruvec_init(%struct.lruvec* noundef %lruvec) local_unnamed_addr #3 {
entry:
  %0 = bitcast %struct.lruvec* %lruvec to i8*
  %call = call i8* @memset(i8* noundef %0, i32 noundef 0, i64 noundef 136) #7
  %.compoundliteral.sroa.0.0..sroa_idx = getelementptr inbounds %struct.lruvec, %struct.lruvec* %lruvec, i64 0, i32 1, i32 0, i32 0, i32 0, i32 0, i32 0, i32 0
  store i32 0, i32* %.compoundliteral.sroa.0.0..sroa_idx, align 8
  %arrayidx = getelementptr %struct.lruvec, %struct.lruvec* %lruvec, i64 0, i32 0, i64 0
  call fastcc void @INIT_LIST_HEAD(%struct.list_head* noundef %arrayidx) #6
  %arrayidx.1 = getelementptr %struct.lruvec, %struct.lruvec* %lruvec, i64 0, i32 0, i64 1
  call fastcc void @INIT_LIST_HEAD(%struct.list_head* noundef %arrayidx.1) #6
  %arrayidx.2 = getelementptr %struct.lruvec, %struct.lruvec* %lruvec, i64 0, i32 0, i64 2
  call fastcc void @INIT_LIST_HEAD(%struct.list_head* noundef %arrayidx.2) #6
  %arrayidx.3 = getelementptr %struct.lruvec, %struct.lruvec* %lruvec, i64 0, i32 0, i64 3
  call fastcc void @INIT_LIST_HEAD(%struct.list_head* noundef %arrayidx.3) #6
  %arrayidx.4 = getelementptr %struct.lruvec, %struct.lruvec* %lruvec, i64 0, i32 0, i64 4
  call fastcc void @INIT_LIST_HEAD(%struct.list_head* noundef %arrayidx.4) #6
  ret void
}
