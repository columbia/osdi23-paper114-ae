; ModuleID = 'arch/arm64/mm/ioremap.c'
source_filename = "arch/arm64/mm/ioremap.c"
...
define dso_local i8* @__ioremap(i64 noundef %phys_addr, i64 noundef %size, i64 %prot.coerce) local_unnamed_addr #0 {
entry:
  ...
  %call = call fastcc i8* @__ioremap_caller(i64 noundef %phys_addr, i64 noundef %size, i64 %prot.coerce, i8* noundef %3) #4
  ret i8* %call
}

; Function Attrs: noinline nounwind null_pointer_is_valid
define internal fastcc i8* @__ioremap_caller(i64 noundef %phys_addr, i64 noundef %size, i64 %prot.coerce, i8* noundef %caller) unnamed_addr #0 {
entry:
  %and = and i64 %phys_addr, 4095
  %and1 = and i64 %phys_addr, -4096
  %add = add i64 %size, 4095
  %add2 = add i64 %add, %and
  %and3 = and i64 %add2, -4096
  %add4 = add i64 %and1, -1
  %sub = add i64 %add4, %and3
  %tobool.not = icmp ne i64 %and3, 0
  %cmp = icmp uge i64 %sub, %and1
  %or.cond.not = and i1 %tobool.not, %cmp
  %tobool7.not = icmp ult i64 %sub, 281474976710656
  %or.cond69 = and i1 %tobool7.not, %or.cond.not
  br i1 %or.cond69, label %if.end, label %cleanup

if.end:                                           ; preds = %entry
  %shr = lshr i64 %phys_addr, 12
  %call = call i32 @pfn_is_map_memory(i64 noundef %shr) #5
  %tobool8.not = icmp eq i32 %call, 0
  br i1 %tobool8.not, label %if.end27, label %if.then16, !prof !7

if.then16:                                        ; preds = %if.end
  call void asm sideeffect ".pushsection __bug_table,\22aw\22; .align 2; 14470: .long 14471f - 14470b; .pushsection .rodata.str,\22aMS\22,@progbits,1; 14472: .string \22arch/arm64/mm/ioremap.c\22; .popsection; .long 14472b - 14470b; .short 46; .short (1 << 0)|(((9) << 8)); .popsection; 14471: brk 0x800", ""() #6, !srcloc !8
  br label %cleanup

if.end27:                                         ; preds = %if.end
  %call28 = call %struct.vm_struct* @get_vm_area_caller(i64 noundef %and3, i64 noundef 1, i8* noundef %caller) #5
  %tobool29.not = icmp eq %struct.vm_struct* %call28, null
  br i1 %tobool29.not, label %cleanup, label %if.end31

if.end31:                                         ; preds = %if.end27
  %addr32 = getelementptr inbounds %struct.vm_struct, %struct.vm_struct* %call28, i64 0, i32 1
  %0 = load i8*, i8** %addr32, align 8
  %1 = ptrtoint i8* %0 to i64
  %phys_addr33 = getelementptr inbounds %struct.vm_struct, %struct.vm_struct* %call28, i64 0, i32 6
  store i64 %and1, i64* %phys_addr33, align 8
  %add34 = add i64 %and3, %1
  %call36 = call i32 @ioremap_page_range(i64 noundef %1, i64 noundef %add34, i64 noundef %and1, i64 %prot.coerce) #5
  %tobool37.not = icmp eq i32 %call36, 0
  br i1 %tobool37.not, label %if.end39, label %if.then38

if.then38:                                        ; preds = %if.end31
  call void @vunmap(i8* noundef %0) #5
  br label %cleanup

if.end39:                                         ; preds = %if.end31
  %add40 = add i64 %and, %1
  %2 = inttoptr i64 %add40 to i8*
  br label %cleanup

cleanup:                                          ; preds = %if.then16, %if.end27, %entry, %if.end39, %if.then38
  %retval.0 = phi i8* [ null, %if.then38 ], [ %2, %if.end39 ], [ null, %entry ], [ null, %if.then16 ], [ null, %if.end27 ]
  ret i8* %retval.0
}
...
; Function Attrs: noinline nounwind null_pointer_is_valid
define dso_local void @iounmap(i8* noundef %io_addr) local_unnamed_addr #0 {
entry:
  %0 = ptrtoint i8* %io_addr to i64
  %and = and i64 %0, -4096
  %1 = inttoptr i64 %and to i8*
  %call = call i1 @is_vmalloc_addr(i8* noundef %1) #5
  br i1 %call, label %if.then, label %if.end

if.then:                                          ; preds = %entry
  call void @vunmap(i8* noundef %1) #5
  br label %if.end
...
; Function Attrs: null_pointer_is_valid
declare dso_local void @vunmap(i8* noundef) local_unnamed_addr #2

; Function Attrs: noinline nounwind null_pointer_is_valid
define dso_local i8* @ioremap_cache(i64 noundef %phys_addr, i64 noundef %size) local_unnamed_addr #0 {
entry:
  %shr = lshr i64 %phys_addr, 12
  %call = call i32 @pfn_is_map_memory(i64 noundef %shr) #5
...
if.then:                                          ; preds = %entry
  ...
  %sub = sub i64 %phys_addr, %0
  %or = or i64 %sub, -549755813888
  %1 = inttoptr i64 %or to i8*
  br label %return

if.end:                                           ; preds = %entry
  %2 = load i8, i8* @arm64_use_ng_mappings, align 1, !range !9
  %tobool1.not = icmp eq i8 %2, 0
  %or5 = select i1 %tobool1.not, i64 29273397577910019, i64 29273397577912067
  ...
  %shl12.neg = shl nsw i64 -1, %5
  %or11 = or i64 %shl12.neg, %4
  %and15 = and i64 %shl12.neg, 36028797018963967
  %neg = xor i64 %and15, -1
  %and17 = and i64 %neg, %4
  %cond18 = select i1 %tobool7.not, i64 %and17, i64 %or11
  %6 = inttoptr i64 %cond18 to i8*
  %call19 = call fastcc i8* @__ioremap_caller(i64 noundef %phys_addr, i64 noundef %size, i64 %or5, i8* noundef %6) #4
  br label %return

return:                                           ; preds = %if.end, %if.then
  %retval.0 = phi i8* [ %1, %if.then ], [ %call19, %if.end ]
  ret i8* %retval.0
}
...
define dso_local void @early_ioremap_init() local_unnamed_addr #3 section ".init.text" {
entry:
  call void @early_ioremap_setup() #5
  ret void
}
...
; Function Attrs: null_pointer_is_valid
declare dso_local %struct.vm_struct* @get_vm_area_caller(i64 noundef, i64 noundef, i8* noundef) local_unnamed_addr #2
