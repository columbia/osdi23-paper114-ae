; ModuleID = 'arch/arm64/kernel/return_address.c'
source_filename = "arch/arm64/kernel/return_address.c"
target datalayout = "e-m:e-i8:8:32-i16:16:32-i64:64-i128:128-n32:64-S128"
target triple = "aarch64-unknown-linux-gnu"

%struct.return_address_data = type { i32, i8* }
...
define dso_local i8* @return_address(i32 noundef %level) #0 {
entry:
  %data = alloca %struct.return_address_data, align 8
  %frame = alloca %struct.stackframe, align 8
  %0 = bitcast %struct.return_address_data* %data to i8*
  call void @llvm.lifetime.start.p0i8(i64 16, i8* nonnull %0) #6
  %1 = bitcast %struct.return_address_data* %data to i64*, !annotation !7
  store i64 0, i64* %1, align 8, !annotation !7
  %2 = bitcast %struct.stackframe* %frame to i8*
  ...
  %add = add i32 %level, 2
  %level1 = getelementptr inbounds %struct.return_address_data, %struct.return_address_data* %data, i64 0, i32 0
  store i32 %add, i32* %level1, align 8
  %addr = getelementptr inbounds %struct.return_address_data, %struct.return_address_data* %data, i64 0, i32 1
  store i8* null, i8** %addr, align 8
  %3 = call i8* @llvm.frameaddress.p0i8(i32 0)
  %4 = ptrtoint i8* %3 to i64
  call void @start_backtrace(%struct.stackframe* noundef nonnull %frame, i64 noundef %4, i64 noundef ptrtoint (i8* (i32)* @return_address to i64)) #7
  %5 = call i64 asm "mrs $0, sp_el0", "=r"() #8, !srcloc !8
  %6 = inttoptr i64 %5 to %struct.task_struct*
  call void @walk_stackframe(%struct.task_struct* noundef %6, %struct.stackframe* noundef nonnull %frame, i1 (i8*, i64)* noundef nonnull @save_return_addr, i8* noundef nonnull %0) #7
  %7 = load i32, i32* %level1, align 8
  %tobool.not = icmp eq i32 %7, 0
  %8 = load i8*, i8** %addr, align 8
  %retval.0 = select i1 %tobool.not, i8* %8, i8* null
...
; Function Attrs: null_pointer_is_valid
declare dso_local void @walk_stackframe(%struct.task_struct* noundef, %struct.stackframe* noundef, i1 (i8*, i64)* noundef, i8* noundef) local_unnamed_addr #3

; Function Attrs: mustprogress nofree noinline norecurse nosync nounwind null_pointer_is_valid willreturn
define internal i1 @save_return_addr(i8* nocapture noundef %d, i64 noundef %pc) #5 {
entry:
  %level = bitcast i8* %d to i32*
  %0 = load i32, i32* %level, align 8
  %tobool.not = icmp eq i32 %0, 0
  br i1 %tobool.not, label %if.then, label %if.else

if.then:                                          ; preds = %entry
  %1 = inttoptr i64 %pc to i8*
  %addr = getelementptr inbounds i8, i8* %d, i64 8
  %2 = bitcast i8* %addr to i8**
  store i8* %1, i8** %2, align 8
  br label %cleanup

if.else:                                          ; preds = %entry
  %dec = add i32 %0, -1
  store i32 %dec, i32* %level, align 8
  br label %cleanup
