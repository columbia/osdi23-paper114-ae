; ModuleID = 'arch/arm64/kernel/time.c'
source_filename = "arch/arm64/kernel/time.c"
...
@.str = private unnamed_addr constant [41 x i8] c"Unable to initialise architected timer.\0A\00", align 1
@lpj_fine = external dso_local local_unnamed_addr global i64, align 8

; Function Attrs: noinline nounwind null_pointer_is_valid
define dso_local i64 @profile_pc(%struct.pt_regs* nocapture noundef readonly %regs) local_unnamed_addr #0 {
entry:
  ...
  %call = call i32 @in_lock_functions(i64 noundef %2) #7
...
if.end:                                           ; preds = %entry
  ...
  call void @start_backtrace(%struct.stackframe* noundef nonnull %frame, i64 noundef %4, i64 noundef %5) #7
  %pc7 = getelementptr inbounds %struct.stackframe, %struct.stackframe* %frame, i64 0, i32 1
  br label %do.body

do.body:                                          ; preds = %do.cond, %if.end
  %call4 = call i32 @unwind_frame(%struct.task_struct* noundef null, %struct.stackframe* noundef nonnull %frame) #7
  %cmp = icmp slt i32 %call4, 0
  br i1 %cmp, label %cleanup11, label %do.cond

do.cond:                                          ; preds = %do.body
  %6 = load i64, i64* %pc7, align 8
  %call8 = call i32 @in_lock_functions(i64 noundef %6) #7
  %tobool9.not = icmp eq i32 %call8, 0
  br i1 %tobool9.not, label %do.end, label %do.body

do.end:                                           ; preds = %do.cond
  %7 = load i64, i64* %pc7, align 8
  br label %cleanup11

cleanup11:                                        ; preds = %do.body, %do.end, %if.then
  %retval.2 = phi i64 [ %7, %do.end ], [ %3, %if.then ], [ 0, %do.body ]
...
declare dso_local i32 @in_lock_functions(i64 noundef) local_unnamed_addr #3
...
define dso_local void @time_init() local_unnamed_addr #4 section ".init.text" {
entry:
  call void @of_clk_init(%struct.of_device_id* noundef null) #7
  call void @timer_probe() #7
  %call = call i32 @arch_timer_get_rate() #7
  %tobool.not = icmp eq i32 %call, 0
  br i1 %tobool.not, label %if.then, label %if.end

if.then:                                          ; preds = %entry
  call void (i8*, ...) @panic(i8* noundef getelementptr inbounds ([41 x i8], [41 x i8]* @.str, i64 0, i64 0)) #8
  unreachable

if.end:                                           ; preds = %entry
  %div = udiv i32 %call, 250
  %conv = zext i32 %div to i64
  store i64 %conv, i64* @lpj_fine, align 8
  ret void
}

; Function Attrs: null_pointer_is_valid
declare dso_local void @of_clk_init(%struct.of_device_id* noundef) local_unnamed_addr #3
...
attributes #8 = { cold nobuiltin noreturn nounwind "no-builtins" }
