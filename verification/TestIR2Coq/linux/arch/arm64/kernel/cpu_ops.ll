; ModuleID = 'arch/arm64/kernel/cpu_ops.c'
source_filename = "arch/arm64/kernel/cpu_ops.c"
...
@cpu_ops = internal unnamed_addr global [256 x %struct.cpu_operations*] zeroinitializer, section ".data..ro_after_init", align 8
@.str = private unnamed_addr constant [33 x i8] c"\014Unsupported enable-method: %s\0A\00", align 1
@.str.1 = private unnamed_addr constant [43 x i8] c"\013Failed to find device node for boot cpu\0A\00", align 1
@.str.2 = private unnamed_addr constant [14 x i8] c"enable-method\00", align 1
@.str.3 = private unnamed_addr constant [40 x i8] c"\013%pOF: missing enable-method property\0A\00", align 1
@dt_supported_cpu_ops = internal unnamed_addr constant [3 x %struct.cpu_operations*] [%struct.cpu_operations* @smp_spin_table_ops, %struct.cpu_operations* @cpu_psci_ops, %struct.cpu_operations* null], section ".init.rodata", align 8
@smp_spin_table_ops = external dso_local constant %struct.cpu_operations, align 8
@cpu_psci_ops = external dso_local constant %struct.cpu_operations, align 8

; Function Attrs: cold noinline nounwind null_pointer_is_valid optsize
define dso_local i32 @init_cpu_ops(i32 noundef %cpu) local_unnamed_addr #0 section ".init.text" {
entry:
  %call = call fastcc i8* @cpu_read_enable_method(i32 noundef %cpu) #4
  %tobool.not = icmp eq i8* %call, null
  br i1 %tobool.not, label %cleanup, label %if.end

if.end:                                           ; preds = %entry
  %call1 = call fastcc %struct.cpu_operations* @cpu_get_ops(i8* noundef nonnull %call) #4
  %idxprom = sext i32 %cpu to i64
  %arrayidx = getelementptr [256 x %struct.cpu_operations*], [256 x %struct.cpu_operations*]* @cpu_ops, i64 0, i64 %idxprom
  store %struct.cpu_operations* %call1, %struct.cpu_operations** %arrayidx, align 8
  %tobool4.not = icmp eq %struct.cpu_operations* %call1, null
  br i1 %tobool4.not, label %do.end, label %cleanup

do.end:                                           ; preds = %if.end
  %call6 = call i32 (i8*, ...) @_printk(i8* noundef getelementptr inbounds ([33 x i8], [33 x i8]* @.str, i64 0, i64 0), i8* noundef nonnull %call) #5
  br label %cleanup

cleanup:                                          ; preds = %if.end, %entry, %do.end
  %retval.0 = phi i32 [ -95, %do.end ], [ -19, %entry ], [ 0, %if.end ]
  ret i32 %retval.0
}

; Function Attrs: cold noinline nounwind null_pointer_is_valid optsize
define internal fastcc i8* @cpu_read_enable_method(i32 noundef %cpu) unnamed_addr #0 section ".init.text" {
entry:
  %call = call %struct.device_node* @of_get_cpu_node(i32 noundef %cpu, i32* noundef null) #6
  %tobool.not = icmp eq %struct.device_node* %call, null
  br i1 %tobool.not, label %if.then, label %if.end4

if.then:                                          ; preds = %entry
  %tobool1.not = icmp eq i32 %cpu, 0
  br i1 %tobool1.not, label %do.end, label %cleanup

do.end:                                           ; preds = %if.then
  %call3 = call i32 (i8*, ...) @_printk(i8* noundef getelementptr inbounds ([43 x i8], [43 x i8]* @.str.1, i64 0, i64 0)) #5
  br label %cleanup

if.end4:                                          ; preds = %entry
  %call5 = call i8* @of_get_property(%struct.device_node* noundef nonnull %call, i8* noundef getelementptr inbounds ([14 x i8], [14 x i8]* @.str.2, i64 0, i64 0), i32* noundef null) #6
  %tobool6 = icmp eq i8* %call5, null
  %cmp = icmp ne i32 %cpu, 0
  %or.cond = and i1 %cmp, %tobool6
  br i1 %or.cond, label %do.end11, label %cleanup

do.end11:                                         ; preds = %if.end4
  %call13 = call i32 (i8*, ...) @_printk(i8* noundef getelementptr inbounds ([40 x i8], [40 x i8]* @.str.3, i64 0, i64 0), %struct.device_node* noundef nonnull %call) #5
  br label %cleanup

cleanup:                                          ; preds = %if.end4, %do.end11, %if.then, %do.end
  %spec.select = phi i8* [ %call5, %if.end4 ], [ null, %do.end11 ], [ null, %if.then ], [ null, %do.end ]
  ret i8* %spec.select
}

; Function Attrs: cold noinline nounwind null_pointer_is_valid optsize
define internal fastcc %struct.cpu_operations* @cpu_get_ops(i8* noundef %name) unnamed_addr #0 section ".init.text" {
entry:
  br label %while.body

while.body:                                       ; preds = %entry, %if.end
  %0 = phi %struct.cpu_operations* [ @smp_spin_table_ops, %entry ], [ %3, %if.end ]
  %ops.08 = phi %struct.cpu_operations** [ getelementptr inbounds ([3 x %struct.cpu_operations*], [3 x %struct.cpu_operations*]* @dt_supported_cpu_ops, i64 0, i64 0), %entry ], [ %incdec.ptr, %if.end ]
  %name1 = getelementptr inbounds %struct.cpu_operations, %struct.cpu_operations* %0, i64 0, i32 0
  %1 = load i8*, i8** %name1, align 8
  %call = call i32 @strcmp(i8* noundef %name, i8* noundef %1) #6
  %tobool2.not = icmp eq i32 %call, 0
  br i1 %tobool2.not, label %if.then, label %if.end

if.then:                                          ; preds = %while.body
  %2 = load %struct.cpu_operations*, %struct.cpu_operations** %ops.08, align 8
  br label %cleanup

if.end:                                           ; preds = %while.body
  %incdec.ptr = getelementptr %struct.cpu_operations*, %struct.cpu_operations** %ops.08, i64 1
  %3 = load %struct.cpu_operations*, %struct.cpu_operations** %incdec.ptr, align 8
  %tobool.not = icmp eq %struct.cpu_operations* %3, null
  br i1 %tobool.not, label %cleanup, label %while.body

cleanup:                                          ; preds = %if.end, %if.then
  %retval.0 = phi %struct.cpu_operations* [ %2, %if.then ], [ null, %if.end ]
  ret %struct.cpu_operations* %retval.0
}
...
define dso_local %struct.cpu_operations* @get_cpu_ops(i32 noundef %cpu) local_unnamed_addr #2 {
entry:
  %idxprom = sext i32 %cpu to i64
  %arrayidx = getelementptr [256 x %struct.cpu_operations*], [256 x %struct.cpu_operations*]* @cpu_ops, i64 0, i64 %idxprom
  %0 = load %struct.cpu_operations*, %struct.cpu_operations** %arrayidx, align 8
  ret %struct.cpu_operations* %0
}
...
attributes #4 = { cold nobuiltin "no-builtins" }
