; ModuleID = 'drivers/clocksource/timer-probe.c'
...
@__timer_of_table = external dso_local global [0 x %struct.of_device_id], align 8
...
@__timer_of_table_sentinel = internal constant %struct.of_device_id zeroinitializer, section "__timer_of_table_end", align 8
@llvm.compiler.used = appending global [1 x i8*] [i8* getelementptr inbounds (%struct.of_device_id, %struct.of_device_id* @__timer_of_table_sentinel, i32 0, i32 0, i32 0)], section "llvm.metadata"
...
entry:
  ...
  %call = call %struct.device_node* @of_find_matching_node_and_match(%struct.device_node* noundef null, %struct.of_device_id* noundef getelementptr inbounds ([0 x %struct.of_device_id], [0 x %struct.of_device_id]* @__timer_of_table, i64 0, i64 0), %struct.of_device_id** noundef nonnull %match) #5
  %tobool.not32 = icmp eq %struct.device_node* %call, null
  br i1 %tobool.not32, label %do.end15, label %for.body

for.body:                                         ; preds = %entry, %for.inc
  %np.034 = phi %struct.device_node* [ %call9, %for.inc ], [ %call, %entry ]
  %timers.033 = phi i32 [ %timers.1, %for.inc ], [ 0, %entry ]
  %call1 = call i1 @of_device_is_available(%struct.device_node* noundef nonnull %np.034) #5
  br i1 %call1, label %if.end, label %for.inc

if.end:                                           ; preds = %for.body
  %1 = load %struct.of_device_id*, %struct.of_device_id** %match, align 8
  ...
  %call2 = call i32 %3(%struct.device_node* noundef nonnull %np.034) #5
  switch i32 %call2, label %do.end [
    i32 0, label %if.end8
    i32 -517, label %for.inc
  ]

do.end:                                           ; preds = %if.end
  %call6 = call i32 (i8*, ...) @_printk(i8* noundef getelementptr inbounds ([35 x i8], [35 x i8]* @.str, i64 0, i64 0), %struct.device_node* noundef nonnull %np.034, i32 noundef %call2) #6
  br label %for.inc

if.end8:                                          ; preds = %if.end
  %inc = add i32 %timers.033, 1
  br label %for.inc

for.inc:                                          ; preds = %if.end, %do.end, %for.body, %if.end8
  %timers.1 = phi i32 [ %timers.033, %do.end ], [ %inc, %if.end8 ], [ %timers.033, %for.body ], [ %timers.033, %if.end ]
  %call9 = call %struct.device_node* @of_find_matching_node_and_match(%struct.device_node* noundef nonnull %np.034, %struct.of_device_id* noundef getelementptr inbounds ([0 x %struct.of_device_id], [0 x %struct.of_device_id]* @__timer_of_table, i64 0, i64 0), %struct.of_device_id** noundef nonnull %match) #5
  %tobool.not = icmp eq %struct.device_node* %call9, null
...
do.end15:                                         ; preds = %entry, %for.end
  %call17 = call i32 (i8*, ...) @_printk(i8* noundef getelementptr inbounds ([32 x i8], [32 x i8]* @.str.1, i64 0, i64 0), i8* noundef getelementptr inbounds ([12 x i8], [12 x i8]* @__func__.timer_probe, i64 0, i64 0)) #6
  br label %if.end18

if.end18:                                         ; preds = %do.end15, %for.end
...
declare dso_local %struct.device_node* @of_find_matching_node_and_match(%struct.device_node* noundef, %struct.of_device_id* noundef, %struct.of_device_id** noundef) local_unnamed_addr #2
