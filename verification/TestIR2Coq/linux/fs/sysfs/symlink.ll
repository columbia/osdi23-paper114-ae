...
define dso_local i32 @sysfs_create_link_sd(%struct.kernfs_node* noundef %kn, %struct.kobject* nocapture noundef readonly %target, i8* noundef %name) local_unnamed_addr #0 {
entry:
  %call = call fastcc i32 @sysfs_do_create_link_sd(%struct.kernfs_node* noundef %kn, %struct.kobject* noundef %target, i8* noundef %name, i32 noundef 1) #4
  ret i32 %call
}

; Function Attrs: noinline nounwind null_pointer_is_valid
define internal fastcc i32 @sysfs_do_create_link_sd(%struct.kernfs_node* noundef %parent, %struct.kobject* nocapture noundef readonly %target_kobj, i8* noundef %name, i32 noundef %warn) unnamed_addr #0 {
entry:
  %tobool.not = icmp eq i8* %name, null
  %tobool1.not = icmp eq %struct.kernfs_node* %parent, null
  %0 = or i1 %tobool1.not, %tobool.not
  br i1 %0, label %if.then, label %if.end19, !prof !7

if.then:                                          ; preds = %entry
  call void asm sideeffect ".pushsection __bug_table,\22aw\22; .align 2; 14470: .long 14471f - 14470b; .pushsection .rodata.str,\22aMS\22,@progbits,1; 14472: .string \22fs/sysfs/symlink.c\22; .popsection; .long 14472b - 14470b; .short 26; .short (1 << 0)|(((9) << 8)); .popsection; 14471: brk 0x800", ""() #5, !srcloc !8
  br label %cleanup

if.end19:                                         ; preds = %entry
  call fastcc void @__raw_spin_lock() #6
  ...
  %tobool20.not = icmp eq %struct.kernfs_node* %1, null
  br i1 %tobool20.not, label %if.end23.thread, label %if.end26

if.end23.thread:                                  ; preds = %if.end19
  call fastcc void @__raw_spin_unlock() #6
  br label %cleanup

if.end26:                                         ; preds = %if.end19
  call void @kernfs_get(%struct.kernfs_node* noundef nonnull %1) #6
  call fastcc void @__raw_spin_unlock() #6
  %call = call %struct.kernfs_node* @kernfs_create_link(%struct.kernfs_node* noundef nonnull %parent, i8* noundef nonnull %name, %struct.kernfs_node* noundef nonnull %1) #6
  call void @kernfs_put(%struct.kernfs_node* noundef nonnull %1) #6
  %2 = bitcast %struct.kernfs_node* %call to i8*
  %call27 = call fastcc i1 @IS_ERR(i8* noundef %2) #4
  br i1 %call27, label %if.end29, label %cleanup

if.end29:                                         ; preds = %if.end26
  %tobool30.not = icmp ne i32 %warn, 0
  %.pre = call fastcc i64 @PTR_ERR(i8* noundef %2) #4
  %cmp = icmp eq i64 %.pre, -17
  %or.cond = select i1 %tobool30.not, i1 %cmp, i1 false
  br i1 %or.cond, label %if.then33, label %if.end34

if.then33:                                        ; preds = %if.end29
  call void @sysfs_warn_dup(%struct.kernfs_node* noundef nonnull %parent, i8* noundef nonnull %name) #6
  br label %if.end34

if.end34:                                         ; preds = %if.end29, %if.then33
  %call35.pre-phi = phi i64 [ -17, %if.then33 ], [ %.pre, %if.end29 ]
  %conv36 = trunc i64 %call35.pre-phi to i32
  br label %cleanup

cleanup:                                          ; preds = %if.end23.thread, %if.then, %if.end26, %if.end34
  %retval.0 = phi i32 [ %conv36, %if.end34 ], [ -22, %if.then ], [ 0, %if.end26 ], [ -2, %if.end23.thread ]
  ret i32 %retval.0
}

; Function Attrs: noinline nounwind null_pointer_is_valid
define dso_local i32 @sysfs_create_link(%struct.kobject* noundef %kobj, %struct.kobject* nocapture noundef readonly %target, i8* noundef %name) local_unnamed_addr #0 {
entry:
  %call = call fastcc i32 @sysfs_do_create_link(%struct.kobject* noundef %kobj, %struct.kobject* noundef %target, i8* noundef %name, i32 noundef 1) #4
  ret i32 %call
}

; Function Attrs: noinline nounwind null_pointer_is_valid
define internal fastcc i32 @sysfs_do_create_link(%struct.kobject* noundef readonly %kobj, %struct.kobject* nocapture noundef readonly %target, i8* noundef %name, i32 noundef %warn) unnamed_addr #0 {
entry:
  %tobool.not = icmp eq %struct.kobject* %kobj, null
  %sd = getelementptr inbounds %struct.kobject, %struct.kobject* %kobj, i64 0, i32 5
  %parent.0.in = select i1 %tobool.not, %struct.kernfs_node** @sysfs_root_kn, %struct.kernfs_node** %sd
  %parent.0 = load %struct.kernfs_node*, %struct.kernfs_node** %parent.0.in, align 8
  %tobool1.not = icmp eq %struct.kernfs_node* %parent.0, null
  br i1 %tobool1.not, label %cleanup, label %if.end3

if.end3:                                          ; preds = %entry
  %call = call fastcc i32 @sysfs_do_create_link_sd(%struct.kernfs_node* noundef nonnull %parent.0, %struct.kobject* noundef %target, i8* noundef %name, i32 noundef %warn) #4
  br label %cleanup

cleanup:                                          ; preds = %entry, %if.end3
  %retval.0 = phi i32 [ %call, %if.end3 ], [ -14, %entry ]
  ret i32 %retval.0
}

; Function Attrs: noinline nounwind null_pointer_is_valid
define dso_local i32 @sysfs_create_link_nowarn(%struct.kobject* noundef %kobj, %struct.kobject* nocapture noundef readonly %target, i8* noundef %name) local_unnamed_addr #0 {
entry:
  %call = call fastcc i32 @sysfs_do_create_link(%struct.kobject* noundef %kobj, %struct.kobject* noundef %target, i8* noundef %name, i32 noundef 0) #4
  ret i32 %call
}

; Function Attrs: noinline nounwind null_pointer_is_valid
define dso_local void @sysfs_delete_link(%struct.kobject* nocapture noundef readonly %kobj, %struct.kobject* nocapture noundef readonly %targ, i8* noundef %name) local_unnamed_addr #0 {
entry:
  call fastcc void @__raw_spin_lock() #6
  %sd = getelementptr inbounds %struct.kobject, %struct.kobject* %targ, i64 0, i32 5
...
land.lhs.true:                                    ; preds = %entry
  ...
  %call = call fastcc i1 @kernfs_ns_enabled(%struct.kernfs_node* noundef %1) #4
  br i1 %call, label %if.then, label %if.end

if.then:                                          ; preds = %land.lhs.true
  %ns3 = getelementptr inbounds %struct.kernfs_node, %struct.kernfs_node* %0, i64 0, i32 5
  %2 = load i8*, i8** %ns3, align 8
  br label %if.end

if.end:                                           ; preds = %if.then, %land.lhs.true, %entry
  %ns.0 = phi i8* [ %2, %if.then ], [ null, %land.lhs.true ], [ null, %entry ]
  call fastcc void @__raw_spin_unlock() #6
  %sd4 = getelementptr inbounds %struct.kobject, %struct.kobject* %kobj, i64 0, i32 5
  %3 = load %struct.kernfs_node*, %struct.kernfs_node** %sd4, align 8
  %call5 = call i32 @kernfs_remove_by_name_ns(%struct.kernfs_node* noundef %3, i8* noundef %name, i8* noundef %ns.0) #6
  ret void
}

; Function Attrs: mustprogress nofree noinline norecurse nosync nounwind null_pointer_is_valid readonly willreturn
define internal fastcc i1 @kernfs_ns_enabled(%struct.kernfs_node* nocapture noundef readonly %kn) unnamed_addr #1 {
entry:
...
; Function Attrs: noinline nounwind null_pointer_is_valid
define dso_local void @sysfs_remove_link(%struct.kobject* noundef readonly %kobj, i8* noundef %name) local_unnamed_addr #0 {
entry:
  %tobool.not = icmp eq %struct.kobject* %kobj, null
  %sd = getelementptr inbounds %struct.kobject, %struct.kobject* %kobj, i64 0, i32 5
  %parent.0.in = select i1 %tobool.not, %struct.kernfs_node** @sysfs_root_kn, %struct.kernfs_node** %sd
  %parent.0 = load %struct.kernfs_node*, %struct.kernfs_node** %parent.0.in, align 8
  call fastcc void @kernfs_remove_by_name(%struct.kernfs_node* noundef %parent.0, i8* noundef %name) #4
  ret void
}
...
entry:
  %call = call i32 @kernfs_remove_by_name_ns(%struct.kernfs_node* noundef %parent, i8* noundef %name, i8* noundef null) #6
  ret void
}

; Function Attrs: noinline nounwind null_pointer_is_valid
define dso_local i32 @sysfs_rename_link_ns(%struct.kobject* noundef readonly %kobj, %struct.kobject* noundef readonly %targ, i8* noundef %old, i8* noundef %new, i8* noundef %new_ns) local_unnamed_addr #0 {
entry:
  %tobool.not = icmp eq %struct.kobject* %kobj, null
  %sd = getelementptr inbounds %struct.kobject, %struct.kobject* %kobj, i64 0, i32 5
  %parent.0.in = select i1 %tobool.not, %struct.kernfs_node** @sysfs_root_kn, %struct.kernfs_node** %sd
  %parent.0 = load %struct.kernfs_node*, %struct.kernfs_node** %parent.0.in, align 8
  %sd1 = getelementptr inbounds %struct.kobject, %struct.kobject* %targ, i64 0, i32 5
  %0 = load %struct.kernfs_node*, %struct.kernfs_node** %sd1, align 8
  %tobool2.not = icmp eq %struct.kernfs_node* %0, null
  br i1 %tobool2.not, label %if.end5, label %if.then3

if.then3:                                         ; preds = %entry
  %ns = getelementptr inbounds %struct.kernfs_node, %struct.kernfs_node* %0, i64 0, i32 5
  %1 = load i8*, i8** %ns, align 8
  br label %if.end5

if.end5:                                          ; preds = %if.then3, %entry
  %old_ns.0 = phi i8* [ %1, %if.then3 ], [ null, %entry ]
  %call = call %struct.kernfs_node* @kernfs_find_and_get_ns(%struct.kernfs_node* noundef %parent.0, i8* noundef %old, i8* noundef %old_ns.0) #6
  %tobool6.not = icmp eq %struct.kernfs_node* %call, null
  br i1 %tobool6.not, label %out, label %if.end8

if.end8:                                          ; preds = %if.end5
  %call9 = call fastcc i32 @kernfs_type(%struct.kernfs_node* noundef nonnull %call) #4
  %cmp.not = icmp eq i32 %call9, 4
  br i1 %cmp.not, label %if.end11, label %out
...
if.end11:                                         ; preds = %if.end8
  ...
  %3 = load %struct.kernfs_node*, %struct.kernfs_node** %target_kn, align 8
  %priv = getelementptr inbounds %struct.kernfs_node, %struct.kernfs_node* %3, i64 0, i32 8
  %4 = load i8*, i8** %priv, align 8
  %5 = bitcast %struct.kobject* %targ to i8*
  %cmp12.not = icmp eq i8* %4, %5
  br i1 %cmp12.not, label %if.end14, label %out

if.end14:                                         ; preds = %if.end11
  %call15 = call i32 @kernfs_rename_ns(%struct.kernfs_node* noundef nonnull %call, %struct.kernfs_node* noundef %parent.0, i8* noundef %new, i8* noundef %new_ns) #6
  br label %out

out:                                              ; preds = %if.end11, %if.end8, %if.end5, %if.end14
  %result.0 = phi i32 [ -22, %if.end8 ], [ -22, %if.end11 ], [ %call15, %if.end14 ], [ -2, %if.end5 ]
  call void @kernfs_put(%struct.kernfs_node* noundef %call) #6
  ret i32 %result.0
}
...
; Function Attrs: mustprogress nofree noinline norecurse nosync nounwind null_pointer_is_valid readonly willreturn
define internal fastcc i32 @kernfs_type(%struct.kernfs_node* nocapture noundef readonly %kn) unnamed_addr #1 {
entry:
  %flags = getelementptr inbounds %struct.kernfs_node, %struct.kernfs_node* %kn, i64 0, i32 10
  %0 = load i16, i16* %flags, align 8
  %1 = and i16 %0, 15
...
; Function Attrs: null_pointer_is_valid
declare dso_local i32 @kernfs_rename_ns(%struct.kernfs_node* noundef, %struct.kernfs_node* noundef, i8* noundef, i8* noundef) local_unnamed_addr #2
...
entry:
  ...
  call fastcc void @do_raw_spin_lock() #4
  ret void
}
...
if.end.i:                                         ; preds = %entry
  call void @queued_spin_lock_slowpath(%struct.qspinlock* noundef getelementptr inbounds (%struct.spinlock, %struct.spinlock* @sysfs_symlink_target_lock, i64 0, i32 0, i32 0, i32 0), i32 noundef %call11.i.i.i.i) #6
...
entry:
  call void asm sideeffect "stlrb ${1:w}, $0", "=*Q,r,~{memory}"(i8* elementtype(i8) bitcast (%struct.spinlock* @sysfs_symlink_target_lock to i8*), i8 0) #5, !srcloc !13
  ret void
}
...
!11 = !{i64 2148029423, i64 2148029456, i64 2148029509, i64 2148029568, i64 2148029602, i64 2148029657, i64 2148029686, i64 2148029706}
!12 = !{i64 2149543919}
!13 = !{i64 2149336149}
