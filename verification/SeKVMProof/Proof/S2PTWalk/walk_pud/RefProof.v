...
Section S2PTWalk_walk_pud_RefProof.

  Context `{int_ptr: IntPtrCast}.

  Definition walk_pud_spec_mid (v_vmid: Z) (v_pgd: Z) (v_addr: Z) (v_alloc: Z) (st: RData) : (option (Z * RData)) :=
  ...
  Hint Unfold walk_pud_spec_mid: spec.

  Lemma f_walk_pud_refine_mid:
    forall v_vmid v_pgd v_addr v_alloc lst hst hst' res
           (Hrel: refrel hst lst)
           (Hspec: walk_pud_spec_mid v_vmid v_pgd v_addr v_alloc hst = Some (res, hst')),
      exists lst', walk_pud_spec_low v_vmid v_pgd v_addr v_alloc lst = Some (res, lst') /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
  ...
  Lemma f_walk_pud_refine_high:
    forall v_vmid v_pgd v_addr v_alloc lst hst hst' res
           (Hrel: refrel hst lst)
           (Hspec: walk_pud_spec v_vmid v_pgd v_addr v_alloc hst = Some (res, hst')),
      exists lst', walk_pud_spec_mid v_vmid v_pgd v_addr v_alloc lst = Some (res, lst') /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
      autounfold with spec in *; autounfold with sem in *; simpl in *.
  ...
  Lemma f_walk_pud_refine:
    ...
    Proof.
      intros; inv Hrel.
      eapply f_walk_pud_refine_high in Hspec; try unfold refrel; try reflexivity.
      destruct Hspec as (lst' & Hspec & Hrel).
      inv Hrel; try unfold refrel; try reflexivity.
      eapply f_walk_pud_refine_mid; try unfold refrel; try reflexivity; try eassumption.
    Qed.

End S2PTWalk_walk_pud_RefProof.
