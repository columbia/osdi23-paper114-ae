...
Parameter S2PTWalk_init: RData.

Section S2PTWalk_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition S2PTWalk_load (size: Z) (p: Ptr) (st: RData) : option (Z * RData) := None.

  Definition S2PTWalk_store (size: Z) (p: Ptr) (val: Z) (st: RData) : option RData := None.

  Definition S2PTWalk_alloca (size: Z) (align: Z) (st: RData) : option (Ptr * RData) := None.

  Definition S2PTWalk_free (p: Ptr) (st: RData) : option RData := None.

  Definition S2PTWalk_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition S2PTWalk_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition S2PTWalk_get_flag (f: flag) (st: RData) : bool := false.

  Definition S2PTWalk_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition S2PTWalk_layer :=
    {|
      State := RData;
      Init := S2PTWalk_init;
      Load := S2PTWalk_load;
      Store := S2PTWalk_store;
      Alloca := S2PTWalk_alloca;
      Free := S2PTWalk_free;
      GetReg := S2PTWalk_get_reg;
      SetReg := S2PTWalk_set_reg;
      GetFlag := S2PTWalk_get_flag;
      SetFlag := S2PTWalk_set_flag;
      PrimCall :=
