...
Section SmmuMemOps_assign_smmu_RefProof.

  Context `{int_ptr: IntPtrCast}.

  Definition assign_smmu_spec_mid (v_vmid: Z) (v_pfn: Z) (v_gfn: Z) (st: RData) : (option RData) :=
  ...
  Hint Unfold assign_smmu_spec_mid: spec.

  Lemma f_assign_smmu_refine_mid:
    forall v_vmid v_pfn v_gfn lst hst hst'
           (Hrel: refrel hst lst)
           (Hspec: assign_smmu_spec_mid v_vmid v_pfn v_gfn hst = Some hst'),
      exists lst', assign_smmu_spec_low v_vmid v_pfn v_gfn lst = Some lst' /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
  ...
  Lemma f_assign_smmu_refine_high:
    forall v_vmid v_pfn v_gfn lst hst hst'
           (Hrel: refrel hst lst)
           (Hspec: assign_smmu_spec v_vmid v_pfn v_gfn hst = Some hst'),
      exists lst', assign_smmu_spec_mid v_vmid v_pfn v_gfn lst = Some lst' /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
      autounfold with spec in *; autounfold with sem in *; simpl in *.
  ...
  Lemma f_assign_smmu_refine:
    ...
    Proof.
      intros; inv Hrel.
      eapply f_assign_smmu_refine_high in Hspec; try unfold refrel; try reflexivity.
      destruct Hspec as (lst' & Hspec & Hrel).
      inv Hrel; try unfold refrel; try reflexivity.
      eapply f_assign_smmu_refine_mid; try unfold refrel; try reflexivity; try eassumption.
    Qed.

End SmmuMemOps_assign_smmu_RefProof.
