...
Parameter SmmuMemOps_init: RData.

Section SmmuMemOps_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition SmmuMemOps_load (size: Z) (p: Ptr) (st: RData) : option (Z * RData) := None.

  Definition SmmuMemOps_store (size: Z) (p: Ptr) (val: Z) (st: RData) : option RData := None.

  Definition SmmuMemOps_alloca (size: Z) (align: Z) (st: RData) : option (Ptr * RData) := None.

  Definition SmmuMemOps_free (p: Ptr) (st: RData) : option RData := None.

  Definition SmmuMemOps_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition SmmuMemOps_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition SmmuMemOps_get_flag (f: flag) (st: RData) : bool := false.

  Definition SmmuMemOps_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition SmmuMemOps_layer :=
    {|
      State := RData;
      Init := SmmuMemOps_init;
      Load := SmmuMemOps_load;
      Store := SmmuMemOps_store;
      Alloca := SmmuMemOps_alloca;
      Free := SmmuMemOps_free;
      GetReg := SmmuMemOps_get_reg;
      SetReg := SmmuMemOps_set_reg;
      GetFlag := SmmuMemOps_get_flag;
      SetFlag := SmmuMemOps_set_flag;
      PrimCall :=
