...
Parameter SmmuPTAlloc_init: RData.

Section SmmuPTAlloc_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition SmmuPTAlloc_load (size: Z) (p: Ptr) (st: RData) : option (Z * RData) := None.

  Definition SmmuPTAlloc_store (size: Z) (p: Ptr) (val: Z) (st: RData) : option RData := None.

  Definition SmmuPTAlloc_alloca (size: Z) (align: Z) (st: RData) : option (Ptr * RData) := None.

  Definition SmmuPTAlloc_free (p: Ptr) (st: RData) : option RData := None.

  Definition SmmuPTAlloc_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition SmmuPTAlloc_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition SmmuPTAlloc_get_flag (f: flag) (st: RData) : bool := false.

  Definition SmmuPTAlloc_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition SmmuPTAlloc_layer :=
    {|
      State := RData;
      Init := SmmuPTAlloc_init;
      Load := SmmuPTAlloc_load;
      Store := SmmuPTAlloc_store;
      Alloca := SmmuPTAlloc_alloca;
      Free := SmmuPTAlloc_free;
      GetReg := SmmuPTAlloc_get_reg;
      SetReg := SmmuPTAlloc_set_reg;
      GetFlag := SmmuPTAlloc_get_flag;
      SetFlag := SmmuPTAlloc_set_flag;
      PrimCall :=
          ...
          :: ("alloc_smmu_pgd_page", prim alloc_smmu_pgd_page_spec)
          ...
          :: ("alloc_smmu_pmd_page", prim alloc_smmu_pmd_page_spec)
