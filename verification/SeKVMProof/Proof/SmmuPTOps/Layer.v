...
Parameter SmmuPTOps_init: RData.

Section SmmuPTOps_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition SmmuPTOps_load (size: Z) (p: Ptr) (st: RData) : option (Z * RData) := None.

  Definition SmmuPTOps_store (size: Z) (p: Ptr) (val: Z) (st: RData) : option RData := None.

  Definition SmmuPTOps_alloca (size: Z) (align: Z) (st: RData) : option (Ptr * RData) := None.

  Definition SmmuPTOps_free (p: Ptr) (st: RData) : option RData := None.

  Definition SmmuPTOps_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition SmmuPTOps_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition SmmuPTOps_get_flag (f: flag) (st: RData) : bool := false.

  Definition SmmuPTOps_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition SmmuPTOps_layer :=
    {|
      State := RData;
      Init := SmmuPTOps_init;
      Load := SmmuPTOps_load;
      Store := SmmuPTOps_store;
      Alloca := SmmuPTOps_alloca;
      Free := SmmuPTOps_free;
      GetReg := SmmuPTOps_get_reg;
      SetReg := SmmuPTOps_set_reg;
      GetFlag := SmmuPTOps_get_flag;
      SetFlag := SmmuPTOps_set_flag;
      PrimCall :=
