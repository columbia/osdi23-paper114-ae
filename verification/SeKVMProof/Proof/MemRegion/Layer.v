...
Parameter MemRegion_init: RData.

Section MemRegion_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition MemRegion_load (size: Z) (p: Ptr) (st: RData) : option (Z * RData) := None.

  Definition MemRegion_store (size: Z) (p: Ptr) (val: Z) (st: RData) : option RData := None.

  Definition MemRegion_alloca (size: Z) (align: Z) (st: RData) : option (Ptr * RData) := None.

  Definition MemRegion_free (p: Ptr) (st: RData) : option RData := None.

  Definition MemRegion_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition MemRegion_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition MemRegion_get_flag (f: flag) (st: RData) : bool := false.

  Definition MemRegion_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition MemRegion_layer :=
    {|
      State := RData;
      Init := MemRegion_init;
      Load := MemRegion_load;
      Store := MemRegion_store;
      Alloca := MemRegion_alloca;
      Free := MemRegion_free;
      GetReg := MemRegion_get_reg;
      SetReg := MemRegion_set_reg;
      GetFlag := MemRegion_get_flag;
      SetFlag := MemRegion_set_flag;
      PrimCall :=
          ...
          :: ("mem_region_search", prim mem_region_search_spec)
