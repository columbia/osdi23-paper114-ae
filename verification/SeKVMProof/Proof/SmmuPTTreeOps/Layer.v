...
Parameter SmmuPTTreeOps_init: RData.

Section SmmuPTTreeOps_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition SmmuPTTreeOps_load (size: Z) (p: Ptr) (st: RData) : option (Z * RData) := None.

  Definition SmmuPTTreeOps_store (size: Z) (p: Ptr) (val: Z) (st: RData) : option RData := None.

  Definition SmmuPTTreeOps_alloca (size: Z) (align: Z) (st: RData) : option (Ptr * RData) := None.

  Definition SmmuPTTreeOps_free (p: Ptr) (st: RData) : option RData := None.

  Definition SmmuPTTreeOps_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition SmmuPTTreeOps_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition SmmuPTTreeOps_get_flag (f: flag) (st: RData) : bool := false.

  Definition SmmuPTTreeOps_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition SmmuPTTreeOps_layer :=
    {|
      State := RData;
      Init := SmmuPTTreeOps_init;
      Load := SmmuPTTreeOps_load;
      Store := SmmuPTTreeOps_store;
      Alloca := SmmuPTTreeOps_alloca;
      Free := SmmuPTTreeOps_free;
      GetReg := SmmuPTTreeOps_get_reg;
      SetReg := SmmuPTTreeOps_set_reg;
      GetFlag := SmmuPTTreeOps_get_flag;
      SetFlag := SmmuPTTreeOps_set_flag;
      PrimCall :=
          ...
          :: ("walk_smmu_pt", prim walk_smmu_pt_spec)
          ...
          :: ("set_smmu_pt", prim set_smmu_pt_spec)
          ...
          :: ("init_smmu_pt", prim init_smmu_pt_spec)
