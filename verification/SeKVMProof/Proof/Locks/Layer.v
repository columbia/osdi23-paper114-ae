...
Parameter Locks_init: RData.

Section Locks_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition Locks_load (size: Z) (p: Ptr) (st: RData) : option (Z * RData) := None.

  Definition Locks_store (size: Z) (p: Ptr) (val: Z) (st: RData) : option RData := None.

  Definition Locks_alloca (size: Z) (align: Z) (st: RData) : option (Ptr * RData) := None.

  Definition Locks_free (p: Ptr) (st: RData) : option RData := None.

  Definition Locks_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition Locks_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition Locks_get_flag (f: flag) (st: RData) : bool := false.

  Definition Locks_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition Locks_layer :=
    {|
      State := RData;
      Init := Locks_init;
      Load := Locks_load;
      ...
      Alloca := Locks_alloca;
      Free := Locks_free;
      GetReg := Locks_get_reg;
      SetReg := Locks_set_reg;
      GetFlag := Locks_get_flag;
      SetFlag := Locks_set_flag;
      PrimCall :=
          ...
          :: ("pool_start", prim pool_start_spec)
          ...
          :: ("set_pud_next", prim set_pud_next_spec)
          ...
          :: ("set_pgd_next", prim set_pgd_next_spec)
          ...
          :: ("pmd_pool_end", prim pmd_pool_end_spec)
          ...
          :: ("set_pt_vttbr", prim set_pt_vttbr_spec)
          ...
          :: ("release_lock_pt", prim release_lock_pt_spec)
          ...
          :: ("pud_pool_end", prim pud_pool_end_spec)
          ...
          :: ("pgd_pool_end", prim pgd_pool_end_spec)
          :: ("set_pmd_next", prim set_pmd_next_spec)
