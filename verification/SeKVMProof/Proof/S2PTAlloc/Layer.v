...
Parameter S2PTAlloc_init: RData.

Section S2PTAlloc_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition S2PTAlloc_load (size: Z) (p: Ptr) (st: RData) : option (Z * RData) := None.

  Definition S2PTAlloc_store (size: Z) (p: Ptr) (val: Z) (st: RData) : option RData := None.

  Definition S2PTAlloc_alloca (size: Z) (align: Z) (st: RData) : option (Ptr * RData) := None.

  Definition S2PTAlloc_free (p: Ptr) (st: RData) : option RData := None.

  Definition S2PTAlloc_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition S2PTAlloc_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition S2PTAlloc_get_flag (f: flag) (st: RData) : bool := false.

  Definition S2PTAlloc_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition S2PTAlloc_layer :=
    {|
      State := RData;
      Init := S2PTAlloc_init;
      Load := S2PTAlloc_load;
      Store := S2PTAlloc_store;
      Alloca := S2PTAlloc_alloca;
      Free := S2PTAlloc_free;
      GetReg := S2PTAlloc_get_reg;
      SetReg := S2PTAlloc_set_reg;
      GetFlag := S2PTAlloc_get_flag;
      SetFlag := S2PTAlloc_set_flag;
      PrimCall :=
          ...
          :: ("alloc_s2pt_pgd", prim alloc_s2pt_pgd_spec)
          ...
          :: ("alloc_s2pt_pmd", prim alloc_s2pt_pmd_spec)
          ...
          :: ("alloc_s2pt_pud", prim alloc_s2pt_pud_spec)
