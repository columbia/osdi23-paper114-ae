...
Section PageManager_get_pfn_owner_RefProof.
  ...
  Lemma f_get_pfn_owner_refine:
    forall _pfn lst hst hst' res
      (Hrel: refrel hst lst)
      (Hspec: get_pfn_owner_spec _pfn hst = Some (res, hst')),
    exists lst', get_pfn_owner_spec_low _pfn lst = Some (res, lst') /\ refrel hst' lst'.
  Proof.
    intros. pose proof Hrel as REL. inv Hrel. inv same_rel. inv s2page_rel; simpl in *.
    unfold get_pfn_owner_spec, get_pfn_owner_spec_low in *.
    ...
    apply M1 in idx_inv. grewrite.
    eexists. split. reflexivity. assumption.
    destruct Hidx as [|Hidx]. lia.
    unfold get_s2_page_vmid_spec.
