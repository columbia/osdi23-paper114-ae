...
Require Import PageManager.get_pfn_count.LowSpec.

Local Open Scope string_scope.
Local Open Scope Z_scope.
Local Opaque Z.add Z.mul Z.div Z.sub Z.land Z.lor Z.lxor Z.shiftl Z.shiftr Z.quot Z.rem.

Section PageManager_get_pfn_count_RefProof.

  Context `{int_ptr: IntPtrCast}.

  Lemma f_get_pfn_count_refine:
    forall _pfn lst hst hst' res
      (Hrel: refrel hst lst)
      (Hspec: get_pfn_count_spec _pfn hst = Some (res, hst')),
    exists lst', get_pfn_count_spec_low _pfn lst = Some (res, lst') /\ refrel hst' lst'.
  Proof.
    intros. pose proof Hrel as REL. inv Hrel. inv same_rel. inv s2page_rel; simpl in *.
    unfold get_pfn_count_spec, get_pfn_count_spec_low in *.
    ...
    apply M2 in idx_inv. grewrite.
    eexists. split. reflexivity. assumption.
    destruct Hidx as [|Hidx]. lia.
    unfold get_s2_page_count_spec.
...
End PageManager_get_pfn_count_RefProof.
