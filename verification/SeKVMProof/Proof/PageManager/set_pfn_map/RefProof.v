...
Section PageManager_set_pfn_map_RefProof.

  Context `{int_ptr: IntPtrCast}.

  Lemma f_set_pfn_map_refine:
    forall _pfn _gfn lst hst hst'
      (Hrel: refrel hst lst)
      (Hspec: set_pfn_map_spec _pfn _gfn hst = Some hst'),
    exists lst', set_pfn_map_spec_low _pfn _gfn lst = Some lst' /\ refrel hst' lst'.
  Proof.
    intros. inv Hrel. inv same_rel. inv s2page_rel; simpl in *.
    unfold set_pfn_map_spec, set_pfn_map_spec_low in *.
...
End PageManager_set_pfn_map_RefProof.
