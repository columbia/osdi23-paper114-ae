...
Parameter PageManager_init: RData.

Section PageManager_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition PageManager_load (size: Z) (p: Ptr) (st: RData) : option (Z * RData) := None.

  Definition PageManager_store (size: Z) (p: Ptr) (val: Z) (st: RData) : option RData := None.

  Definition PageManager_alloca (size: Z) (align: Z) (st: RData) : option (Ptr * RData) := None.

  Definition PageManager_free (p: Ptr) (st: RData) : option RData := None.

  Definition PageManager_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition PageManager_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition PageManager_get_flag (f: flag) (st: RData) : bool := false.

  Definition PageManager_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition PageManager_layer :=
    {|
      State := RData;
      Init := PageManager_init;
      Load := PageManager_load;
      Store := PageManager_store;
      Alloca := PageManager_alloca;
      Free := PageManager_free;
      GetReg := PageManager_get_reg;
      SetReg := PageManager_set_reg;
      GetFlag := PageManager_get_flag;
      SetFlag := PageManager_set_flag;
      PrimCall :=
