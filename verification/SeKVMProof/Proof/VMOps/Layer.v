...
Parameter VMOps_init: RData.

Section VMOps_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition VMOps_load (size: Z) (p: Ptr) (st: RData) : option (Z * RData) := None.

  Definition VMOps_store (size: Z) (p: Ptr) (val: Z) (st: RData) : option RData := None.

  Definition VMOps_alloca (size: Z) (align: Z) (st: RData) : option (Ptr * RData) := None.

  Definition VMOps_free (p: Ptr) (st: RData) : option RData := None.

  Definition VMOps_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition VMOps_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition VMOps_get_flag (f: flag) (st: RData) : bool := false.

  Definition VMOps_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition VMOps_layer :=
    {|
      State := RData;
      Init := VMOps_init;
      Load := VMOps_load;
      ...
      Alloca := VMOps_alloca;
      Free := VMOps_free;
      GetReg := VMOps_get_reg;
      SetReg := VMOps_set_reg;
      GetFlag := VMOps_get_flag;
      SetFlag := VMOps_set_flag;
      PrimCall :=
