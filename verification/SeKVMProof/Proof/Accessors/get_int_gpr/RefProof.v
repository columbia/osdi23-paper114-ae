...
Section Accessors_get_int_gpr_RefProof.

  Context `{int_ptr: IntPtrCast}.

  Definition get_int_gpr_spec_mid (v_vmid: Z) (v_vcpuid: Z) (v_index: Z) (st: RData) : (option (Z * RData)) :=
  ...
  Hint Unfold get_int_gpr_spec_mid: spec.

  Lemma f_get_int_gpr_refine_mid:
    forall v_vmid v_vcpuid v_index lst hst hst' res
           (Hrel: refrel hst lst)
           (Hspec: get_int_gpr_spec_mid v_vmid v_vcpuid v_index hst = Some (res, hst')),
      exists lst', get_int_gpr_spec_low v_vmid v_vcpuid v_index lst = Some (res, lst') /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
  ...
  Lemma f_get_int_gpr_refine_high:
    forall v_vmid v_vcpuid v_index lst hst hst' res
           (Hrel: refrel hst lst)
           (Hspec: get_int_gpr_spec v_vmid v_vcpuid v_index hst = Some (res, hst')),
      exists lst', get_int_gpr_spec_mid v_vmid v_vcpuid v_index lst = Some (res, lst') /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
      autounfold with spec in *; autounfold with sem in *; simpl in *.
  ...
  Lemma f_get_int_gpr_refine:
    ...
    Proof.
      intros; inv Hrel.
      eapply f_get_int_gpr_refine_high in Hspec; try unfold refrel; try reflexivity.
      destruct Hspec as (lst' & Hspec & Hrel).
      inv Hrel; try unfold refrel; try reflexivity.
      eapply f_get_int_gpr_refine_mid; try unfold refrel; try reflexivity; try eassumption.
    Qed.

End Accessors_get_int_gpr_RefProof.
