...
Require Import Accessors.get_smmu_pgd_next.LowSpec.
...
Section Accessors_get_smmu_pgd_next_RefProof.

  Context `{int_ptr: IntPtrCast}.

  Definition get_smmu_pgd_next_spec_mid (st: RData) : (option (Z * RData)) :=
    (anno (((33731152 * 0) = 0));
  ...
  Hint Unfold get_smmu_pgd_next_spec_mid: spec.

  Lemma f_get_smmu_pgd_next_refine_mid:
    forall lst hst hst' res
           (Hrel: refrel hst lst)
           (Hspec: get_smmu_pgd_next_spec_mid hst = Some (res, hst')),
      exists lst', get_smmu_pgd_next_spec_low lst = Some (res, lst') /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
  ...
  Lemma f_get_smmu_pgd_next_refine_high:
    forall lst hst hst' res
           (Hrel: refrel hst lst)
           (Hspec: get_smmu_pgd_next_spec hst = Some (res, hst')),
      exists lst', get_smmu_pgd_next_spec_mid lst = Some (res, lst') /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
      autounfold with spec in *; autounfold with sem in *; simpl in *.
  ...
  Lemma f_get_smmu_pgd_next_refine:
    forall lst hst hst' res
           (Hrel: refrel hst lst)
           (Hspec: get_smmu_pgd_next_spec hst = Some (res, hst')),
      exists lst', get_smmu_pgd_next_spec_low lst = Some (res, lst') /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
      eapply f_get_smmu_pgd_next_refine_high in Hspec; try unfold refrel; try reflexivity.
      destruct Hspec as (lst' & Hspec & Hrel).
      inv Hrel; try unfold refrel; try reflexivity.
      eapply f_get_smmu_pgd_next_refine_mid; try unfold refrel; try reflexivity; try eassumption.
    Qed.

End Accessors_get_smmu_pgd_next_RefProof.
