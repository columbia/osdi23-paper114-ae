...
Section Accessors_get_shadow_ctxt_RefProof.

  Context `{int_ptr: IntPtrCast}.

  Definition get_shadow_ctxt_spec_mid (v_vmid: Z) (v_vcpuid: Z) (v_index: Z) (st: RData) : (option (Z * RData)) :=
    if (v_index <? 41)
    then (
  ...
  Hint Unfold get_shadow_ctxt_spec_mid: spec.

  Lemma f_get_shadow_ctxt_refine_mid:
    forall v_vmid v_vcpuid v_index lst hst hst' res
           (Hrel: refrel hst lst)
           (Hspec: get_shadow_ctxt_spec_mid v_vmid v_vcpuid v_index hst = Some (res, hst')),
      exists lst', get_shadow_ctxt_spec_low v_vmid v_vcpuid v_index lst = Some (res, lst') /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
      autounfold with spec in Hspec; autounfold with sem in Hspec;
        simpl in Hspec.
      destruct_spec Hspec; try_solve.
      - unfold get_shadow_ctxt_spec_low; autounfold with sem.
        unfold get_el2_data_start_spec. simpl.
        repeat solve_refproof.
        unfold load_RData; simpl.
        unfold el2_data_lock_map; simpl. repeat solve_refproof.
        autounfold with sem. unfold load_el2_data; simpl.
        repeat solve_refproof.
        unfold load_s_shadow_vcpu_context.
        repeat solve_refproof.
  ...
  Lemma f_get_shadow_ctxt_refine_high:
    forall v_vmid v_vcpuid v_index lst hst hst' res
           (Hrel: refrel hst lst)
           (Hspec: get_shadow_ctxt_spec v_vmid v_vcpuid v_index hst = Some (res, hst')),
      exists lst', get_shadow_ctxt_spec_mid v_vmid v_vcpuid v_index lst = Some (res, lst') /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
  ...
  Lemma f_get_shadow_ctxt_refine:
    forall v_vmid v_vcpuid v_index lst hst hst' res
           (Hrel: refrel hst lst)
           (Hspec: get_shadow_ctxt_spec v_vmid v_vcpuid v_index hst = Some (res, hst')),
      exists lst', get_shadow_ctxt_spec_low v_vmid v_vcpuid v_index lst = Some (res, lst') /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
      eapply f_get_shadow_ctxt_refine_high in Hspec; try unfold refrel; try reflexivity.
      destruct Hspec as (lst' & Hspec & Hrel).
      inv Hrel; try unfold refrel; try reflexivity.
      eapply f_get_shadow_ctxt_refine_mid; try unfold refrel; try reflexivity; try eassumption.
    Qed.

End Accessors_get_shadow_ctxt_RefProof.
