...
Require Import Accessors.get_shadow_dirty_bit.LowSpec.
...
Section Accessors_get_shadow_dirty_bit_RefProof.

  Context `{int_ptr: IntPtrCast}.

  Definition get_shadow_dirty_bit_spec_mid (v_vmid: Z) (v_vcpuid: Z) (st: RData) : (option (Z * RData)) :=
  ...
  Hint Unfold get_shadow_dirty_bit_spec_mid: spec.

  Lemma f_get_shadow_dirty_bit_refine_mid:
    forall v_vmid v_vcpuid lst hst hst' res
           (Hrel: refrel hst lst)
           (Hspec: get_shadow_dirty_bit_spec_mid v_vmid v_vcpuid hst = Some (res, hst')),
      exists lst', get_shadow_dirty_bit_spec_low v_vmid v_vcpuid lst = Some (res, lst') /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
  ...
  Lemma f_get_shadow_dirty_bit_refine_high:
    forall v_vmid v_vcpuid lst hst hst' res
           (Hrel: refrel hst lst)
           (Hspec: get_shadow_dirty_bit_spec v_vmid v_vcpuid hst = Some (res, hst')),
      exists lst', get_shadow_dirty_bit_spec_mid v_vmid v_vcpuid lst = Some (res, lst') /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
      autounfold with spec in *; autounfold with sem in *; simpl in *.
  ...
  Lemma f_get_shadow_dirty_bit_refine:
    forall v_vmid v_vcpuid lst hst hst' res
           (Hrel: refrel hst lst)
           (Hspec: get_shadow_dirty_bit_spec v_vmid v_vcpuid hst = Some (res, hst')),
      exists lst', get_shadow_dirty_bit_spec_low v_vmid v_vcpuid lst = Some (res, lst') /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
      eapply f_get_shadow_dirty_bit_refine_high in Hspec; try unfold refrel; try reflexivity.
      destruct Hspec as (lst' & Hspec & Hrel).
      inv Hrel; try unfold refrel; try reflexivity.
      eapply f_get_shadow_dirty_bit_refine_mid; try unfold refrel; try reflexivity; try eassumption.
    Qed.

End Accessors_get_shadow_dirty_bit_RefProof.
