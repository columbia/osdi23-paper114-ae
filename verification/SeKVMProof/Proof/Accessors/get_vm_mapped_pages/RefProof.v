...
Require Import Accessors.get_vm_mapped_pages.LowSpec.
...
Section Accessors_get_vm_mapped_pages_RefProof.

  Context `{int_ptr: IntPtrCast}.

  Definition get_vm_mapped_pages_spec_mid (v_vmid: Z) (v_load_idx: Z) (st: RData) : (option (Z * RData)) :=
  ...
  Hint Unfold get_vm_mapped_pages_spec_mid: spec.

  Lemma f_get_vm_mapped_pages_refine_mid:
    forall v_vmid v_load_idx lst hst hst' res
           (Hrel: refrel hst lst)
           (Hspec: get_vm_mapped_pages_spec_mid v_vmid v_load_idx hst = Some (res, hst')),
      exists lst', get_vm_mapped_pages_spec_low v_vmid v_load_idx lst = Some (res, lst') /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
  ...
  Lemma f_get_vm_mapped_pages_refine_high:
    forall v_vmid v_load_idx lst hst hst' res
           (Hrel: refrel hst lst)
           (Hspec: get_vm_mapped_pages_spec v_vmid v_load_idx hst = Some (res, hst')),
      exists lst', get_vm_mapped_pages_spec_mid v_vmid v_load_idx lst = Some (res, lst') /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
      autounfold with spec in *; autounfold with sem in *; simpl in *.
  ...
  Lemma f_get_vm_mapped_pages_refine:
    forall v_vmid v_load_idx lst hst hst' res
           (Hrel: refrel hst lst)
           (Hspec: get_vm_mapped_pages_spec v_vmid v_load_idx hst = Some (res, hst')),
      exists lst', get_vm_mapped_pages_spec_low v_vmid v_load_idx lst = Some (res, lst') /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
      eapply f_get_vm_mapped_pages_refine_high in Hspec; try unfold refrel; try reflexivity.
      destruct Hspec as (lst' & Hspec & Hrel).
      inv Hrel; try unfold refrel; try reflexivity.
      eapply f_get_vm_mapped_pages_refine_mid; try unfold refrel; try reflexivity; try eassumption.
    Qed.

End Accessors_get_vm_mapped_pages_RefProof.
