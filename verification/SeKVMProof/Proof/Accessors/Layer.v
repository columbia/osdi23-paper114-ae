...
Parameter Accessors_init: RData.

Section Accessors_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition Accessors_alloca (size: Z) (align: Z) (st: RData) : option (Ptr * RData) := None.

  Definition Accessors_free (p: Ptr) (st: RData) : option RData := None.

  Definition Accessors_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition Accessors_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition Accessors_get_flag (f: flag) (st: RData) : bool := false.

  Definition Accessors_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition Accessors_layer :=
    {|
      State := RData;
      Init := Accessors_init;
      Load := load_RData;
      Store := store_RData;
      Alloca := Accessors_alloca;
      Free := Accessors_free;
      GetReg := Accessors_get_reg;
      SetReg := Accessors_set_reg;
      GetFlag := Accessors_get_flag;
      SetFlag := Accessors_set_flag;
      PrimCall :=
