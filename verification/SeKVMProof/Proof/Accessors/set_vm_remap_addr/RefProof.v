...
Section Accessors_set_vm_remap_addr_RefProof.

  Context `{int_ptr: IntPtrCast}.

  Definition set_vm_remap_addr_spec_mid (v_vmid: Z) (v_load_idx: Z) (v_remap_addr: Z) (st: RData) : (option RData) :=
  ...
  Hint Unfold set_vm_remap_addr_spec_mid: spec.

  Lemma f_set_vm_remap_addr_refine_mid:
    forall v_vmid v_load_idx v_remap_addr lst hst hst'
           (Hrel: refrel hst lst)
           (Hspec: set_vm_remap_addr_spec_mid v_vmid v_load_idx v_remap_addr hst = Some hst'),
      exists lst', set_vm_remap_addr_spec_low v_vmid v_load_idx v_remap_addr lst = Some lst' /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
  ...
  Lemma f_set_vm_remap_addr_refine_high:
    forall v_vmid v_load_idx v_remap_addr lst hst hst'
           (Hrel: refrel hst lst)
           (Hspec: set_vm_remap_addr_spec v_vmid v_load_idx v_remap_addr hst = Some hst'),
      exists lst', set_vm_remap_addr_spec_mid v_vmid v_load_idx v_remap_addr lst = Some lst' /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
      autounfold with spec in *; autounfold with sem in *; simpl in *.
  ...
  Lemma f_set_vm_remap_addr_refine:
    forall v_vmid v_load_idx v_remap_addr lst hst hst'
           (Hrel: refrel hst lst)
           (Hspec: set_vm_remap_addr_spec v_vmid v_load_idx v_remap_addr hst = Some hst'),
      exists lst', set_vm_remap_addr_spec_low v_vmid v_load_idx v_remap_addr lst = Some lst' /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
      eapply f_set_vm_remap_addr_refine_high in Hspec; try unfold refrel; try reflexivity.
      destruct Hspec as (lst' & Hspec & Hrel).
      inv Hrel; try unfold refrel; try reflexivity.
      eapply f_set_vm_remap_addr_refine_mid; try unfold refrel; try reflexivity; try eassumption.
    Qed.

End Accessors_set_vm_remap_addr_RefProof.
