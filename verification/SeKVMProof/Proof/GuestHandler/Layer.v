...
Parameter GuestHandler_init: RData.

Section GuestHandler_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition GuestHandler_load (size: Z) (p: Ptr) (st: RData) : option (Z * RData) := None.

  Definition GuestHandler_store (size: Z) (p: Ptr) (val: Z) (st: RData) : option RData := None.

  Definition GuestHandler_alloca (size: Z) (align: Z) (st: RData) : option (Ptr * RData) := None.

  Definition GuestHandler_free (p: Ptr) (st: RData) : option RData := None.

  Definition GuestHandler_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition GuestHandler_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition GuestHandler_get_flag (f: flag) (st: RData) : bool := false.

  Definition GuestHandler_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition GuestHandler_layer :=
    {|
      State := RData;
      Init := GuestHandler_init;
      Load := GuestHandler_load;
      Store := GuestHandler_store;
      Alloca := GuestHandler_alloca;
      Free := GuestHandler_free;
      GetReg := GuestHandler_get_reg;
      SetReg := GuestHandler_set_reg;
      GetFlag := GuestHandler_get_flag;
      SetFlag := GuestHandler_set_flag;
      PrimCall :=
          ...
          :: ("handle_pvops", prim handle_pvops_spec)
