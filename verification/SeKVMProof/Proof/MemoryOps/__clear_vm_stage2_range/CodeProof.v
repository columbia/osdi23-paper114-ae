...
Section MemoryOps___clear_vm_stage2_range_CodeProof.

  Context `{int_ptr: IntPtrCast}.

  Local Opaque __clear_vm_range_spec.
  Local Opaque get_mem_region_size_spec.
  Local Opaque get_mem_region_base_spec.
  Local Opaque get_mem_region_cnt_spec.
  Local Opaque get_vm_poweron_spec.
  Local Opaque get_mem_region_flag_spec.

    Lemma f___clear_vm_stage2_range_correct:
      forall v_vmid v_size st st'
             (Hspec: __clear_vm_stage2_range_spec_low v_vmid v_size st = Some st'),
        exec_func VMOpsAux_layer code "__clear_vm_stage2_range"
                  [VInt v_vmid; VInt v_size]
                  st st' None.
    Proof.
      intros; simpl_func Hspec; simpl in *; try solve[try_solve];
        try solve[ eapply exec_func_call; try reflexivity; vcgen].
      remember (fbody Code.f___clear_vm_stage2_range) as fcode eqn:Hc; simpl in Hc;
        match type of Hc with
        | context[ILoop ?body] => remember body as LoopBody eqn:Cloop; clear Hc
        end.
      match goal with
      | [H: __clear_vm_stage2_range_loop_low ?n ?_a0 ?_a1 ?_a2 ?_st =
          Some (?_a0', ?_a1', ?_a2', ?_st') |- _] =>
          rename H into Hfinal;
          assert(HLoop: forall te,
                  gets "v_call2" te = Some (VInt _a0) ->
                  gets "v_i_022" te = Some (VInt _a1) ->
                  gets "v_vmid" te = Some (VInt _a2) ->
                  (forall s, inlist s ("v_vmid"::"v_size"::"v_i_022"::"v_cmp321_not"::"v_call2"::"v_or_cond"::"v_cmp1"::"v_cmp"::"v_call"::nil) = false -> gets s te = None) ->
                  exec VMOpsAux_layer code (ILoop LoopBody :: nil) te _st
                      (sets "v_call2" (VInt _a0')
                      (sets "v_i_022" (VInt _a1')
                      (sets "v_vmid" (VInt _a2')
                          te))) out_normal _st')
      end.
      {
        intros te ? ? ? te_all.
        unfold __clear_vm_stage2_range_loop_rank in *.
        match goal with
        | [H: __clear_vm_stage2_range_loop_low (z_to_nat ?n) ?_a0 ?_a1 ?_a2 ?_st =
            ...
            set(Inv := fun t s w => exists _a0'' _a1'' _a2'' _st'',
                        __clear_vm_stage2_range_loop_low (z_to_nat (W0 - w)) _a0 _a1 _a2 _st = Some (_a0'', _a1'', _a2'', _st'')
                        /\ (__clear_vm_stage2_range_loop_rank _a0'' _a1'' _a2'') = w
                        /\ 0 < w <= W0
                        /\ s = _st''
                        /\ (forall p v, gets p
                                        (sets "v_call2" (VInt _a0'')
            ...
            assert(loop_succ: forall W, Z.of_nat W <= W0 -> exists _a0'' _a1'' _a2'' _st'',
                    __clear_vm_stage2_range_loop_low (z_to_nat (W0 - Z.of_nat W)) _a0 _a1 _a2 _st =
                    Some (_a0'', _a1'', _a2'', _st''))
        end.
        ...
        assert(HLoop: Loop VMOpsAux_layer code LoopBody P Q R).
        {
          ...
          - unfold P, Inv. intros ? ? (CC1 & CC2).
            ...
            subst; repeat eexists; try solve[unfold __clear_vm_stage2_range_loop_rank in *; simpl; try_solve].
...
End MemoryOps___clear_vm_stage2_range_CodeProof.
