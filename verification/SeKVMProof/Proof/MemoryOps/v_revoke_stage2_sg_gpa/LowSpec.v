...
Section MemoryOps_v_revoke_stage2_sg_gpa_LowSpec.

  Context `{int_ptr: IntPtrCast}.

  Fixpoint v_revoke_stage2_sg_gpa_loop_low (_N_: nat) (v_addr_addr_044: Z) (v_len_143: Z) (v_vmid: Z) (st: RData) : (option (Z * Z * Z * RData)) :=
    match (_N_) with
    | O => (Some (v_addr_addr_044, v_len_143, v_vmid, st))
    | (S _N_) =>
      match ((v_revoke_stage2_sg_gpa_loop_low _N_ v_addr_addr_044 v_len_143 v_vmid st)) with
      | (Some (v_addr_addr_044, v_len_143, v_vmid, st)) =>
        ...
        when st == (
            ...
            then (Some st)
              ...
              when st == ((revoke_vm_page_spec v_vmid v_pfn_0 st));
              (Some st)));
  ...
  Definition v_revoke_stage2_sg_gpa_spec_low (v_vmid: Z) (v_addr: Z) (v_size: Z) (st: RData) : (option RData) :=
    ...
    when st == (
        ...
        then (Some st)
          ...
          rely (((v_revoke_stage2_sg_gpa_loop_rank v_addr_addr_044 v_len_143 v_vmid) >= 0));
          match ((v_revoke_stage2_sg_gpa_loop_low (z_to_nat (v_revoke_stage2_sg_gpa_loop_rank v_addr_addr_044 v_len_143 v_vmid)) v_addr_addr_044 v_len_143 v_vmid st)) with
...
End MemoryOps_v_revoke_stage2_sg_gpa_LowSpec.

#[global] Hint Unfold v_revoke_stage2_sg_gpa_loop_low: spec.
#[global] Hint Unfold v_revoke_stage2_sg_gpa_spec_low: spec.
