...
Parameter MemoryOpsAux_init: RData.

Section MemoryOpsAux_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition MemoryOpsAux_load (size: Z) (p: Ptr) (st: RData) : option (Z * RData) := None.

  Definition MemoryOpsAux_store (size: Z) (p: Ptr) (val: Z) (st: RData) : option RData := None.

  Definition MemoryOpsAux_alloca (size: Z) (align: Z) (st: RData) : option (Ptr * RData) := None.

  Definition MemoryOpsAux_free (p: Ptr) (st: RData) : option RData := None.

  Definition MemoryOpsAux_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition MemoryOpsAux_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition MemoryOpsAux_get_flag (f: flag) (st: RData) : bool := false.

  Definition MemoryOpsAux_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition MemoryOpsAux_layer :=
    {|
      State := RData;
      Init := MemoryOpsAux_init;
      Load := MemoryOpsAux_load;
      Store := MemoryOpsAux_store;
      Alloca := MemoryOpsAux_alloca;
      Free := MemoryOpsAux_free;
      GetReg := MemoryOpsAux_get_reg;
      SetReg := MemoryOpsAux_set_reg;
      GetFlag := MemoryOpsAux_get_flag;
      SetFlag := MemoryOpsAux_set_flag;
      PrimCall :=
