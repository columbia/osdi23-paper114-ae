...
Parameter SmmuPTWalk_init: RData.

Section SmmuPTWalk_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition SmmuPTWalk_load (size: Z) (p: Ptr) (st: RData) : option (Z * RData) := None.

  Definition SmmuPTWalk_store (size: Z) (p: Ptr) (val: Z) (st: RData) : option RData := None.

  Definition SmmuPTWalk_alloca (size: Z) (align: Z) (st: RData) : option (Ptr * RData) := None.

  Definition SmmuPTWalk_free (p: Ptr) (st: RData) : option RData := None.

  Definition SmmuPTWalk_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition SmmuPTWalk_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition SmmuPTWalk_get_flag (f: flag) (st: RData) : bool := false.

  Definition SmmuPTWalk_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition SmmuPTWalk_layer :=
    {|
      State := RData;
      Init := SmmuPTWalk_init;
      Load := SmmuPTWalk_load;
      Store := SmmuPTWalk_store;
      Alloca := SmmuPTWalk_alloca;
      Free := SmmuPTWalk_free;
      GetReg := SmmuPTWalk_get_reg;
      SetReg := SmmuPTWalk_set_reg;
      GetFlag := SmmuPTWalk_get_flag;
      SetFlag := SmmuPTWalk_set_flag;
      PrimCall :=
          ...
          :: ("walk_smmu_pte", prim walk_smmu_pte_spec)
          ...
          :: ("set_smmu_pte", prim set_smmu_pte_spec)
          ...
          :: ("walk_smmu_pmd", prim walk_smmu_pmd_spec)
          :: ("walk_smmu_pgd", prim walk_smmu_pgd_spec)
