...
Parameter MemManager_init: RData.

Section MemManager_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition MemManager_load (size: Z) (p: Ptr) (st: RData) : option (Z * RData) := None.

  Definition MemManager_store (size: Z) (p: Ptr) (val: Z) (st: RData) : option RData := None.

  Definition MemManager_alloca (size: Z) (align: Z) (st: RData) : option (Ptr * RData) := None.

  Definition MemManager_free (p: Ptr) (st: RData) : option RData := None.

  Definition MemManager_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition MemManager_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition MemManager_get_flag (f: flag) (st: RData) : bool := false.

  Definition MemManager_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition MemManager_layer :=
    {|
      State := RData;
      Init := MemManager_init;
      Load := MemManager_load;
      Store := MemManager_store;
      Alloca := MemManager_alloca;
      Free := MemManager_free;
      GetReg := MemManager_get_reg;
      SetReg := MemManager_set_reg;
      GetFlag := MemManager_get_flag;
      SetFlag := MemManager_set_flag;
      PrimCall :=
          ...
          :: ("clear_vm_page", prim clear_vm_page_spec)
          ...
          :: ("assign_pfn_to_vm", prim assign_pfn_to_vm_spec)
          ...
          :: ("map_pfn_vm", prim map_pfn_vm_spec)
