...
Section MemManager_revoke_vm_page_RefProof.

  Context `{int_ptr: IntPtrCast}.

  Definition revoke_vm_page_spec_mid (v_vmid: Z) (v_pfn: Z) (st: RData) : (option RData) :=
  ...
  Hint Unfold revoke_vm_page_spec_mid: spec.

  Lemma f_revoke_vm_page_refine_mid:
    forall v_vmid v_pfn lst hst hst'
           (Hrel: refrel hst lst)
           (Hspec: revoke_vm_page_spec_mid v_vmid v_pfn hst = Some hst'),
      exists lst', revoke_vm_page_spec_low v_vmid v_pfn lst = Some lst' /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
  ...
  Lemma f_revoke_vm_page_refine_high:
    forall v_vmid v_pfn lst hst hst'
           (Hrel: refrel hst lst)
           (Hspec: revoke_vm_page_spec v_vmid v_pfn hst = Some hst'),
      exists lst', revoke_vm_page_spec_mid v_vmid v_pfn lst = Some lst' /\ refrel hst' lst'.
    Proof.
      Local Transparent revoke_vm_page_spec.
      unfold revoke_vm_page_spec.
      intros; inv Hrel.
      autounfold with spec in *; autounfold with sem in *; simpl in *.
  ...
  Lemma f_revoke_vm_page_refine:
    ...
    Proof.
      ...
      unfold revoke_vm_page_spec.
      intros; inv Hrel.
      eapply f_revoke_vm_page_refine_high in Hspec; try unfold refrel; try reflexivity.
      destruct Hspec as (lst' & Hspec & Hrel).
      inv Hrel; try unfold refrel; try reflexivity.
      eapply f_revoke_vm_page_refine_mid; try unfold refrel; try reflexivity; try eassumption.
    Qed.

End MemManager_revoke_vm_page_RefProof.
