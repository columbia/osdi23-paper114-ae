...
Section MemManager_clear_vm_page_RefProof.

  Context `{int_ptr: IntPtrCast}.

  Definition clear_vm_page_spec_mid (v_vmid: Z) (v_pfn: Z) (st: RData) : (option RData) :=
  ...
  Hint Unfold clear_vm_page_spec_mid: spec.

  Lemma f_clear_vm_page_refine_mid:
    forall v_vmid v_pfn lst hst hst'
           (Hrel: refrel hst lst)
           (Hspec: clear_vm_page_spec_mid v_vmid v_pfn hst = Some hst'),
      exists lst', clear_vm_page_spec_low v_vmid v_pfn lst = Some lst' /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
  ...
  Lemma f_clear_vm_page_refine_high:
    forall v_vmid v_pfn lst hst hst'
           (Hrel: refrel hst lst)
           (Hspec: clear_vm_page_spec v_vmid v_pfn hst = Some hst'),
      exists lst', clear_vm_page_spec_mid v_vmid v_pfn lst = Some lst' /\ refrel hst' lst'.
    Proof.
      Local Transparent clear_vm_page_spec.
      unfold clear_vm_page_spec.
      intros; inv Hrel.
      autounfold with spec in *; autounfold with sem in *; simpl in *.
  ...
  Lemma f_clear_vm_page_refine:
    ...
    Proof.
      ...
      unfold clear_vm_page_spec.
      intros; inv Hrel.
      eapply f_clear_vm_page_refine_high in Hspec; try unfold refrel; try reflexivity.
      destruct Hspec as (lst' & Hspec & Hrel).
      inv Hrel; try unfold refrel; try reflexivity.
      eapply f_clear_vm_page_refine_mid; try unfold refrel; try reflexivity; try eassumption.
    Qed.

End MemManager_clear_vm_page_RefProof.
