...
Section SmmuMemOpsAux_assign_pfn_to_smmu_RefProof.

  Context `{int_ptr: IntPtrCast}.

  Definition assign_pfn_to_smmu_spec_mid (v_vmid: Z) (v_gfn: Z) (v_pfn: Z) (st: RData) : (option RData) :=
  ...
  Hint Unfold assign_pfn_to_smmu_spec_mid: spec.

  Lemma f_assign_pfn_to_smmu_refine_mid:
    forall v_vmid v_gfn v_pfn lst hst hst'
           (Hrel: refrel hst lst)
           (Hspec: assign_pfn_to_smmu_spec_mid v_vmid v_gfn v_pfn hst = Some hst'),
      exists lst', assign_pfn_to_smmu_spec_low v_vmid v_gfn v_pfn lst = Some lst' /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
  ...
  Lemma f_assign_pfn_to_smmu_refine_high:
    forall v_vmid v_gfn v_pfn lst hst hst'
           (Hrel: refrel hst lst)
           (Hspec: assign_pfn_to_smmu_spec v_vmid v_gfn v_pfn hst = Some hst'),
      exists lst', assign_pfn_to_smmu_spec_mid v_vmid v_gfn v_pfn lst = Some lst' /\ refrel hst' lst'.
    Proof.
      Local Transparent assign_pfn_to_smmu_spec.
      unfold assign_pfn_to_smmu_spec.
      intros; inv Hrel.
      autounfold with spec in *; autounfold with sem in *; simpl in *.
  ...
  Lemma f_assign_pfn_to_smmu_refine:
    forall v_vmid v_gfn v_pfn lst hst hst'
           (Hrel: refrel hst lst)
           (Hspec: assign_pfn_to_smmu_spec v_vmid v_gfn v_pfn hst = Some hst'),
      exists lst', assign_pfn_to_smmu_spec_low v_vmid v_gfn v_pfn lst = Some lst' /\ refrel hst' lst'.
    Proof.
      ...
      unfold assign_pfn_to_smmu_spec.
      intros; inv Hrel.
      eapply f_assign_pfn_to_smmu_refine_high in Hspec; try unfold refrel; try reflexivity.
      destruct Hspec as (lst' & Hspec & Hrel).
      inv Hrel; try unfold refrel; try reflexivity.
      eapply f_assign_pfn_to_smmu_refine_mid; try unfold refrel; try reflexivity; try eassumption.
    Qed.

End SmmuMemOpsAux_assign_pfn_to_smmu_RefProof.
