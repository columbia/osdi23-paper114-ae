...
Require Import SmmuOpsAux.smmu_assign_page.LowSpec.
...
Section SmmuOpsAux_smmu_assign_page_RefProof.

  Context `{int_ptr: IntPtrCast}.

  Definition smmu_assign_page_spec_mid (v_cbndx: Z) (v_index: Z) (v_pfn: Z) (v_gfn: Z) (st: RData) : (option RData) :=
  ...
  Hint Unfold smmu_assign_page_spec_mid: spec.

  Lemma f_smmu_assign_page_refine_mid:
    forall v_cbndx v_index v_pfn v_gfn lst hst hst'
           (Hrel: refrel hst lst)
           (Hspec: smmu_assign_page_spec_mid v_cbndx v_index v_pfn v_gfn hst = Some hst'),
      exists lst', smmu_assign_page_spec_low v_cbndx v_index v_pfn v_gfn lst = Some lst' /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
  ...
  Lemma f_smmu_assign_page_refine_high:
    forall v_cbndx v_index v_pfn v_gfn lst hst hst'
           (Hrel: refrel hst lst)
           (Hspec: smmu_assign_page_spec v_cbndx v_index v_pfn v_gfn hst = Some hst'),
      exists lst', smmu_assign_page_spec_mid v_cbndx v_index v_pfn v_gfn lst = Some lst' /\ refrel hst' lst'.
    Proof.
      Local Transparent smmu_assign_page_spec.
      unfold smmu_assign_page_spec.
      intros; inv Hrel.
      autounfold with spec in *; autounfold with sem in *; simpl in *.
  ...
  Lemma f_smmu_assign_page_refine:
    forall v_cbndx v_index v_pfn v_gfn lst hst hst'
           (Hrel: refrel hst lst)
           (Hspec: smmu_assign_page_spec v_cbndx v_index v_pfn v_gfn hst = Some hst'),
      exists lst', smmu_assign_page_spec_low v_cbndx v_index v_pfn v_gfn lst = Some lst' /\ refrel hst' lst'.
    Proof.
      ...
      unfold smmu_assign_page_spec.
      intros; inv Hrel.
      eapply f_smmu_assign_page_refine_high in Hspec; try unfold refrel; try reflexivity.
      destruct Hspec as (lst' & Hspec & Hrel).
      inv Hrel; try unfold refrel; try reflexivity.
      eapply f_smmu_assign_page_refine_mid; try unfold refrel; try reflexivity; try eassumption.
    Qed.

End SmmuOpsAux_smmu_assign_page_RefProof.
