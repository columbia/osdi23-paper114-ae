...
Parameter PageIndex_init: RData.

Section PageIndex_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition PageIndex_load (size: Z) (p: Ptr) (st: RData) : option (Z * RData) := None.

  Definition PageIndex_store (size: Z) (p: Ptr) (val: Z) (st: RData) : option RData := None.

  Definition PageIndex_alloca (size: Z) (align: Z) (st: RData) : option (Ptr * RData) := None.

  Definition PageIndex_free (p: Ptr) (st: RData) : option RData := None.

  Definition PageIndex_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition PageIndex_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition PageIndex_get_flag (f: flag) (st: RData) : bool := false.

  Definition PageIndex_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition PageIndex_layer :=
    {|
      State := RData;
      Init := PageIndex_init;
      Load := PageIndex_load;
      Store := PageIndex_store;
      Alloca := PageIndex_alloca;
      Free := PageIndex_free;
      GetReg := PageIndex_get_reg;
      SetReg := PageIndex_set_reg;
      GetFlag := PageIndex_get_flag;
      SetFlag := PageIndex_set_flag;
      PrimCall :=
          ...
          :: ("get_s2_page_index", prim get_s2_page_index_spec)
