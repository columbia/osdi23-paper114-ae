...
Section Accessors_get_pgd_next_LowSpec.

  Context `{int_ptr: IntPtrCast}.

  Definition get_pgd_next_spec_low (v_vmid: Z) (st: RData) : (option (Z * RData)) :=
    ...
    when v_1, st == ((load_RData 8 v_pud_used_pages st));
    let v_mul := (v_1 << 12) in
    let v_add := (v_0 + 4096) in
    let v_add4 := (v_add + v_mul) in
    let __return__ := true in
    let __retval__ := v_add4 in
    (Some (__retval__, st)).

End Accessors_get_pgd_next_LowSpec.

#[global] Hint Unfold get_pgd_next_spec_low: spec.
