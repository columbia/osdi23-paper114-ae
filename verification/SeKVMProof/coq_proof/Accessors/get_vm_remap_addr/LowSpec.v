...
Section Accessors_get_vm_remap_addr_LowSpec.

  Context `{int_ptr: IntPtrCast}.

  Definition get_vm_remap_addr_spec_low (v_vmid: Z) (v_load_idx: Z) (st: RData) : (option (Z * RData)) :=
    ...
    when v_0, st == ((load_RData 8 v_el2_remap_addr st));
    let __return__ := true in
    let __retval__ := v_0 in
    (Some (__retval__, st)).

End Accessors_get_vm_remap_addr_LowSpec.

#[global] Hint Unfold get_vm_remap_addr_spec_low: spec.
