...
Require Import Accessors.set_vcpu_state.LowSpec.
...
Section Accessors_set_vcpu_state_RefProof.

  Context `{int_ptr: IntPtrCast}.

  Definition set_vcpu_state_spec_mid (v_vmid: Z) (v_vcpuid: Z) (v_state: Z) (st: RData) : (option RData) :=
  ...
  Hint Unfold set_vcpu_state_spec_mid: spec.

  Lemma f_set_vcpu_state_refine_mid:
    forall v_vmid v_vcpuid v_state lst hst hst'
           (Hrel: refrel hst lst)
           (Hspec: set_vcpu_state_spec_mid v_vmid v_vcpuid v_state hst = Some hst'),
      exists lst', set_vcpu_state_spec_low v_vmid v_vcpuid v_state lst = Some lst' /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
  ...
  Lemma f_set_vcpu_state_refine_high:
    forall v_vmid v_vcpuid v_state lst hst hst'
           (Hrel: refrel hst lst)
           (Hspec: set_vcpu_state_spec v_vmid v_vcpuid v_state hst = Some hst'),
      exists lst', set_vcpu_state_spec_mid v_vmid v_vcpuid v_state lst = Some lst' /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
      autounfold with spec in *; autounfold with sem in *; simpl in *.
  ...
  Lemma f_set_vcpu_state_refine:
    ...
    Proof.
      intros; inv Hrel.
      eapply f_set_vcpu_state_refine_high in Hspec; try unfold refrel; try reflexivity.
      destruct Hspec as (lst' & Hspec & Hrel).
      inv Hrel; try unfold refrel; try reflexivity.
      eapply f_set_vcpu_state_refine_mid; try unfold refrel; try reflexivity; try eassumption.
    Qed.

End Accessors_set_vcpu_state_RefProof.
