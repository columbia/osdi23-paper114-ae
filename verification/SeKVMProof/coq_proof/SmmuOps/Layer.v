...
Parameter SmmuOps_init: RData.

Section SmmuOps_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition SmmuOps_load (size: Z) (p: Ptr) (st: RData) : option (Z * RData) := None.

  Definition SmmuOps_store (size: Z) (p: Ptr) (val: Z) (st: RData) : option RData := None.

  Definition SmmuOps_alloca (size: Z) (align: Z) (st: RData) : option (Ptr * RData) := None.

  Definition SmmuOps_free (p: Ptr) (st: RData) : option RData := None.

  Definition SmmuOps_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition SmmuOps_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition SmmuOps_get_flag (f: flag) (st: RData) : bool := false.

  Definition SmmuOps_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition SmmuOps_layer :=
    {|
      State := RData;
      Init := SmmuOps_init;
      Load := SmmuOps_load;
      Store := SmmuOps_store;
      Alloca := SmmuOps_alloca;
      Free := SmmuOps_free;
      GetReg := SmmuOps_get_reg;
      SetReg := SmmuOps_set_reg;
      GetFlag := SmmuOps_get_flag;
      SetFlag := SmmuOps_set_flag;
      PrimCall :=
