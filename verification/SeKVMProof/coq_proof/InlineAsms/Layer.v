...
Parameter InlineAsms_init: RData.

Section InlineAsms_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition InlineAsms_alloca (size: Z) (align: Z) (st: RData) : option (Ptr * RData) := None.

  Definition InlineAsms_free (p: Ptr) (st: RData) : option RData := None.

  Definition InlineAsms_get_flag (f: flag) (st: RData) : bool := false.

  Definition InlineAsms_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition InlineAsms_layer :=
    {|
      State := RData;
      Init := InlineAsms_init;
      Load := load_RData;
      Store := store_RData;
      Alloca := InlineAsms_alloca;
      Free := InlineAsms_free;
      GetReg := get_reg_RData;
      SetReg := set_reg_RData;
      GetFlag := InlineAsms_get_flag;
      SetFlag := InlineAsms_set_flag;
      PrimCall :=
          ...
          :: ("iasm_get_ttbr0_el2", prim iasm_get_ttbr0_el2_spec)
          ...
          :: ("iasm_set_tpidr_el2", prim iasm_set_tpidr_el2_spec)
