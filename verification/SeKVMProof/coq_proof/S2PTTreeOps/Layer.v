...
Parameter S2PTTreeOps_init: RData.

Section S2PTTreeOps_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition S2PTTreeOps_load (size: Z) (p: Ptr) (st: RData) : option (Z * RData) := None.

  Definition S2PTTreeOps_store (size: Z) (p: Ptr) (val: Z) (st: RData) : option RData := None.

  Definition S2PTTreeOps_alloca (size: Z) (align: Z) (st: RData) : option (Ptr * RData) := None.

  Definition S2PTTreeOps_free (p: Ptr) (st: RData) : option RData := None.

  Definition S2PTTreeOps_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition S2PTTreeOps_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition S2PTTreeOps_get_flag (f: flag) (st: RData) : bool := false.

  Definition S2PTTreeOps_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition S2PTTreeOps_layer :=
    {|
      State := RData;
      Init := S2PTTreeOps_init;
      Load := S2PTTreeOps_load;
      Store := S2PTTreeOps_store;
      Alloca := S2PTTreeOps_alloca;
      Free := S2PTTreeOps_free;
      GetReg := S2PTTreeOps_get_reg;
      SetReg := S2PTTreeOps_set_reg;
      GetFlag := S2PTTreeOps_get_flag;
      SetFlag := S2PTTreeOps_set_flag;
      PrimCall :=
