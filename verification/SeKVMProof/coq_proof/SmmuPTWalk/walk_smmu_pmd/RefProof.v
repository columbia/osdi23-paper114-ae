...
Require Import SmmuPTWalk.walk_smmu_pmd.LowSpec.

Local Open Scope string_scope.
Local Open Scope Z_scope.
Local Opaque Z.add Z.mul Z.div Z.sub Z.land Z.lor Z.lxor Z.shiftl Z.shiftr Z.quot Z.rem.

Section SmmuPTWalk_walk_smmu_pmd_RefProof.

  Context `{int_ptr: IntPtrCast}.

  Definition walk_smmu_pmd_spec_mid (v_pgd: Z) (v_addr: Z) (v_alloc: Z) (st: RData) : (option (Z * RData)) :=
  ...
  Hint Unfold walk_smmu_pmd_spec_mid: spec.

  Lemma f_walk_smmu_pmd_refine_mid:
    forall v_pgd v_addr v_alloc lst hst hst' res
           (Hrel: refrel hst lst)
           (Hspec: walk_smmu_pmd_spec_mid v_pgd v_addr v_alloc hst = Some (res, hst')),
      exists lst', walk_smmu_pmd_spec_low v_pgd v_addr v_alloc lst = Some (res, lst') /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
  ...
  Lemma f_walk_smmu_pmd_refine_high:
    forall v_pgd v_addr v_alloc lst hst hst' res
           (Hrel: refrel hst lst)
           (Hspec: walk_smmu_pmd_spec v_pgd v_addr v_alloc hst = Some (res, hst')),
      exists lst', walk_smmu_pmd_spec_mid v_pgd v_addr v_alloc lst = Some (res, lst') /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
      autounfold with spec in *; autounfold with sem in *; simpl in *.
  ...
  Lemma f_walk_smmu_pmd_refine:
    ...
    Proof.
      intros; inv Hrel.
      eapply f_walk_smmu_pmd_refine_high in Hspec; try unfold refrel; try reflexivity.
      destruct Hspec as (lst' & Hspec & Hrel).
      inv Hrel; try unfold refrel; try reflexivity.
      eapply f_walk_smmu_pmd_refine_mid; try unfold refrel; try reflexivity; try eassumption.
    Qed.

End SmmuPTWalk_walk_smmu_pmd_RefProof.
