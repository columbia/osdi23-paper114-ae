...
Section MemoryOpsAux___clear_vm_range_CodeProof.

  Context `{int_ptr: IntPtrCast}.

  Local Opaque clear_vm_page_spec.

    Lemma f___clear_vm_range_correct:
      forall v_vmid v_start v_size st st'
             (Hspec: __clear_vm_range_spec_low v_vmid v_start v_size st = Some st'),
        exec_func MemManager_layer code "__clear_vm_range"
                  [VInt v_vmid; VInt v_start; VInt v_size]
                  st st' None.
    Proof.
      intros; simpl_func Hspec; simpl in *; try solve[try_solve];
        try solve[ eapply exec_func_call; try reflexivity; vcgen].
      remember (fbody Code.f___clear_vm_range) as fcode eqn:Hc; simpl in Hc;
        match type of Hc with
        | context[ILoop ?body] => remember body as LoopBody eqn:Cloop; clear Hc
        end.
      match goal with
      | [H: __clear_vm_range_loop_low ?n ?_a0 ?_a1 ?_a2 ?_st =
          Some (?_a0', ?_a1', ?_a2', ?_st') |- _] =>
          rename H into Hfinal;
          assert(HLoop: forall te,
                  gets "v_num_07" te = Some (VInt _a0) ->
                  gets "v_pfn_06" te = Some (VInt _a1) ->
                  gets "v_vmid" te = Some (VInt _a2) ->
                  (forall s, inlist s ("v_vmid"::"v_start"::"v_size"::"v_pfn_06"::"v_num_07"::"v_shr"::"v_div"::"v_cmp_not5"::nil) = false -> gets s te = None) ->
                  exec MemManager_layer code (ILoop LoopBody :: nil) te _st
                      (sets "v_num_07" (VInt _a0')
                      (sets "v_pfn_06" (VInt _a1')
                      (sets "v_vmid" (VInt _a2')
                          te))) out_normal _st')
      end.
      {
        intros te ? ? ? te_all.
        unfold __clear_vm_range_loop_rank in *.
        match goal with
        | [H: __clear_vm_range_loop_low (z_to_nat ?n) ?_a0 ?_a1 ?_a2 ?_st =
            ...
            set(Inv := fun t s w => exists _a0'' _a1'' _a2'' _st'',
                        __clear_vm_range_loop_low (z_to_nat (W0 - w)) _a0 _a1 _a2 _st = Some (_a0'', _a1'', _a2'', _st'')
                        /\ (__clear_vm_range_loop_rank _a0'' _a1'' _a2'') = w
                        /\ 0 < w <= W0
                        /\ s = _st''
                        /\ (forall p v, gets p
                                        (sets "v_num_07" (VInt _a0'')
            ...
            assert(loop_succ: forall W, Z.of_nat W <= W0 -> exists _a0'' _a1'' _a2'' _st'',
                    __clear_vm_range_loop_low (z_to_nat (W0 - Z.of_nat W)) _a0 _a1 _a2 _st =
                    Some (_a0'', _a1'', _a2'', _st''))
        end.
        ...
        assert(HLoop: Loop MemManager_layer code LoopBody P Q R).
        {
          ...
          - unfold P, Inv. intros ? ? (CC1 & CC2).
            ...
            subst; repeat eexists; try solve[unfold __clear_vm_range_loop_rank in *; simpl; try_solve].
...
End MemoryOpsAux___clear_vm_range_CodeProof.
