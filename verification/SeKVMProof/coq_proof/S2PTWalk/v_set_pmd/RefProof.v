...
Section S2PTWalk_v_set_pmd_RefProof.

  Context `{int_ptr: IntPtrCast}.

  Definition v_set_pmd_spec_mid (v_vmid: Z) (v_pgd: Z) (v_addr: Z) (v_pmd: Z) (st: RData) : (option RData) :=
  ...
  Hint Unfold v_set_pmd_spec_mid: spec.

  Lemma f_v_set_pmd_refine_mid:
    forall v_vmid v_pgd v_addr v_pmd lst hst hst'
           (Hrel: refrel hst lst)
           (Hspec: v_set_pmd_spec_mid v_vmid v_pgd v_addr v_pmd hst = Some hst'),
      exists lst', v_set_pmd_spec_low v_vmid v_pgd v_addr v_pmd lst = Some lst' /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
  ...
  Lemma f_v_set_pmd_refine_high:
    forall v_vmid v_pgd v_addr v_pmd lst hst hst'
           (Hrel: refrel hst lst)
           (Hspec: v_set_pmd_spec v_vmid v_pgd v_addr v_pmd hst = Some hst'),
      exists lst', v_set_pmd_spec_mid v_vmid v_pgd v_addr v_pmd lst = Some lst' /\ refrel hst' lst'.
    Proof.
      intros; inv Hrel.
      autounfold with spec in *; autounfold with sem in *; simpl in *.
  ...
  Lemma f_v_set_pmd_refine:
    ...
    Proof.
      intros; inv Hrel.
      eapply f_v_set_pmd_refine_high in Hspec; try unfold refrel; try reflexivity.
      destruct Hspec as (lst' & Hspec & Hrel).
      inv Hrel; try unfold refrel; try reflexivity.
      eapply f_v_set_pmd_refine_mid; try unfold refrel; try reflexivity; try eassumption.
    Qed.

End S2PTWalk_v_set_pmd_RefProof.
