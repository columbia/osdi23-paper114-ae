...
Section MemoryOps_v_revoke_stage2_sg_gpa_RefProof.

  Context `{int_ptr: IntPtrCast}.

  Fixpoint v_revoke_stage2_sg_gpa_loop_mid (_N_: nat) (v_addr_addr_044: Z) (v_len_143: Z) (v_vmid: Z) (st: RData) : (option (Z * Z * Z * RData)) :=
    match (_N_) with
    | O => (Some (v_addr_addr_044, v_len_143, v_vmid, st))
    | (S _N__0) =>
      match ((v_revoke_stage2_sg_gpa_loop_mid _N__0 v_addr_addr_044 v_len_143 v_vmid st)) with
  ...
  Hint Unfold v_revoke_stage2_sg_gpa_loop_mid: spec.

  Definition v_revoke_stage2_sg_gpa_spec_mid (v_vmid: Z) (v_addr: Z) (v_size: Z) (st: RData) : (option RData) :=
    if (
      if (v_size <? 4096)
      then (
        if ((v_size & 4095) <>? 0)
    ...
    then (Some st)
      ...
      then (
        (anno (((0 + 1) = 1));
        match ((v_revoke_stage2_sg_gpa_loop_mid (z_to_nat 1) v_addr 1 v_vmid st)) with
        ...
        if ((v_size & 4095) <>? 0)
        then (
          match ((v_revoke_stage2_sg_gpa_loop_mid (z_to_nat ((v_size >> 12) + 1)) v_addr ((v_size >> 12) + 1) v_vmid st)) with
          | (Some (v_addr_addr_45, v_len_144, v_vmid_0, st_0)) => (Some st_0)
          | None => None
          end)
        else (
          (anno ((((v_size >> 12) + 0) = (v_size >> 12)));
          match ((v_revoke_stage2_sg_gpa_loop_mid (z_to_nat (v_size >> 12)) v_addr (v_size >> 12) v_vmid st)) with
          | (Some (v_addr_addr_45, v_len_144, v_vmid_0, st_0)) => (Some st_0)
          | None => None
          end)))).

  Hint Unfold v_revoke_stage2_sg_gpa_spec_mid: spec.
