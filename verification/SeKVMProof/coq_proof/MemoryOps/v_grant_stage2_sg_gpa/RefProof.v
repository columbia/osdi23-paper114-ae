...
Section MemoryOps_v_grant_stage2_sg_gpa_RefProof.
  ...
  Proof.
    ...
    intros; inv Hrel.
    simpl in *. Local Opaque v_grant_stage2_sg_gpa_loop_mid v_grant_stage2_sg_gpa_loop_low.
  ...
  Proof.
    ...
    destruct_spec Hspec; repeat solve_refproof;
      repeat eexists; try unfold refrel; solve_equality.
    eapply f_v_grant_stage2_sg_gpa_loop_refine_mid in Hcond2.
  ...
  Proof.
    Local Transparent v_grant_stage2_sg_gpa_loop_mid.
    induction _N_. simpl. intros. inv Hrel. repeat eexists. assumption.
    intros; inv Hrel.
    simpl in *. Local Opaque v_grant_stage2_sg_gpa_loop_mid v_grant_stage2_sg_gpa_loop.
    simpl_hyp Hspec. repeat destruct p. eapply IH_N_ in C.
    destruct C as (lst' & Hloop & Hrel). rewrite Hloop. inv Hrel.
    autounfold with spec in *; autounfold with sem in *; simpl in *.
    destruct_spec Hspec; repeat solve_refproof;
  ...
  Lemma f_v_grant_stage2_sg_gpa_refine_high:
    ...
    Proof.
      ...
      destruct_spec Hspec; repeat (solve_refproof; repeat rewrite annotation_eq);
        repeat eexists; try unfold refrel; solve_equality.
      eapply f_v_grant_stage2_sg_gpa_loop_refine_high in Hcond2.
      destruct Hcond2 as (lst' & Hloop & Hrel).
      rewrite Hloop. inv Hrel. reflexivity. constructor.
      eapply f_v_grant_stage2_sg_gpa_loop_refine_high in Hcond2.
      destruct Hcond2 as (lst' & Hloop & Hrel).
