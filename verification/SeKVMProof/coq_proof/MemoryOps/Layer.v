...
Parameter MemoryOps_init: RData.

Section MemoryOps_Layer.

  Context `{int_ptr: IntPtrCast}.

  Definition MemoryOps_load (size: Z) (p: Ptr) (st: RData) : option (Z * RData) := None.

  Definition MemoryOps_store (size: Z) (p: Ptr) (val: Z) (st: RData) : option RData := None.

  Definition MemoryOps_alloca (size: Z) (align: Z) (st: RData) : option (Ptr * RData) := None.

  Definition MemoryOps_free (p: Ptr) (st: RData) : option RData := None.

  Definition MemoryOps_get_reg (reg: regset) (st: RData) : option Z := None.

  Definition MemoryOps_set_reg (reg: regset) (val: Z) (st: RData) : option RData := None.

  Definition MemoryOps_get_flag (f: flag) (st: RData) : bool := false.

  Definition MemoryOps_set_flag (f: flag) (val: bool) (st: RData) : option RData := None.

  Definition MemoryOps_layer :=
    {|
      State := RData;
      Init := MemoryOps_init;
      Load := MemoryOps_load;
      Store := MemoryOps_store;
      Alloca := MemoryOps_alloca;
      Free := MemoryOps_free;
      GetReg := MemoryOps_get_reg;
      SetReg := MemoryOps_set_reg;
      GetFlag := MemoryOps_get_flag;
      SetFlag := MemoryOps_set_flag;
      PrimCall :=
