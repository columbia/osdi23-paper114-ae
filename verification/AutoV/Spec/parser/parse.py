...
class ProgramVisitor(SpecVisitor):
    ...
    def visitDef(self, ctx: SpecParser.DefContext):
        ...
        elif name == "LAYER_GET_REG" and len(var_anno) == 0 and rettype == String():
            self.current_layer["ops"]["get_reg"] = expr.value
        elif name == "LAYER_SET_REG" and len(var_anno) == 0 and rettype == String():
            self.current_layer["ops"]["set_reg"] = expr.value
        elif name == "LAYER_GET_FLAG" and len(var_anno) == 0 and rettype == String():
            self.current_layer["ops"]["get_flag"] = expr.value
        elif name == "LAYER_SET_FLAG" and len(var_anno) == 0 and rettype == String():
            self.current_layer["ops"]["set_flag"] = expr.value
        else:
            if self.current_layer is not None:
